...
For that reason, we wished to apply network reconstruction to gene expression data from diffuse large B-cell lymphoma samples from our own lab \citep{DybkaerBoegsted2015} as others \citep{Agnelli2011, Clarke2013} have successfully used network analysis in other malignancies.
