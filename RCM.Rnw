...
Of particular interest was the identification of a network with the S100 family of calcium-binding proteins S100A8 (MRP8; calgranulin A) and S100A9 (MRP14; calgranulin B) as central players further fueling indications that knock down of these proteins may improve the immunotherapy strategies and outcome in lymphoma.
\end{abstract}
...
We finally investigated the choice of using only the top \Sexpr{dlbcl.par$top.n} most variable genes and fitted the RMC using the top \Sexpr{dlbcl.par2$top.n} most variable gene.
The EM algorithm estimated an $\Sexpr{dlbcl.par2$top.n} \times \Sexpr{dlbcl.par2$top.n}$ matrix $\hvPsi$ and $\hat{\nu} = \Sexpr{round(dlbcl.rcm2$nu, 1)}$ in
$\Sexpr{dlbcl.rcm2$iterations}$ iterations and $\Sexpr{round(dlbcl.rcm2$time[3]/60, 1)}$ minutes.
This implies an $\text{ICC}(\Sexpr{round(dlbcl.rcm2$nu, 2)}) = \Sexpr{round(with(dlbcl.rcm2, ICC(nu, nrow(Psi))),5)}$, smaller than the above.
The common covariance and correlation matrix was computed from $\hvPsi$ and $\hat{\nu}$ and hierarchical clustering was applied to the results above.
This confirmed that the genes in the \Sexpr{cleanName(the.module)} and
\Sexpr{cleanName(the.other.module)} still largely clustered together.
