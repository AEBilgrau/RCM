...
However, the obvious disadvantage is that the identified maxima might be a local maximum or saddle-point when considering the log-likelihood function jointly in $(\vPsi, \nu)$.
