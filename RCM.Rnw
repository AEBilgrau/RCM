...
Log-likelihood traces and computation times are seen in Figure \ref{\Sexpr{loglik.trace.plot}}.
...
This low ICC might suggest a high study homogeneity of covariances meaning that the covariances reproduce across studies.
...
The approximate MLE and EM algorithms estimated an $\Sexpr{dlbcl.par2$top.n} \times \Sexpr{dlbcl.par2$top.n}$ matrix $\hvPsi$ and $\hat{\nu} = \Sexpr{paste(round(dlbcl.rcm2$nu, 1))}$ in $\Sexpr{round(dlbcl.rcm2$time[3]/3600, 1)}$ hours.
This implies that
$\text{ICC}(\hat{\nu}) = \Sexpr{round(with(dlbcl.rcm2, ICC(nu, nrow(Psi))),5)}$; a somewhat smaller value than the above.
