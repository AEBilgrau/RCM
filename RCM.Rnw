...
For each $n = \Sexpr{min(par.ne$n.obs)}, \ldots, \Sexpr{max(par.ne$n.obs)}$, the weighted sum of squared errors for each estimator, $\text{SSE}(\hvSigma)$, was computed for $\Sexpr{par.ne[["n.sims"]]}$ datasets and the mean and 95\% confidence interval of these values are seen in Figure \ref{\Sexpr{figure1}} as function of the number of samples in each dataset $n_i$.
In the $\Sexpr{par.ne[["n.sims"]]}$ fits, the EM approach used a total of \Sexpr{sim.est.time["time.em.elapsed"]} minutes of computation time compared to \Sexpr{sim.est.time["time.pool.elapsed"]} and \Sexpr{sim.est.time["time.mle.elapsed"]} minutes for the pool and approximate MLE approach, respectively.

\fig{\Sexpr{figure1}}{\textwidth}{
Left: The mean SSE (and 95 \% confidence interval) of \Sexpr{par.ne$n.sims} simulations as a function of the number of samples $n_i$ in each of the \Sexpr{par.ne$k} studies.
Right: The mean computation time of fits with varying dimension $p$.
}

From the left panel of Figure \ref{\Sexpr{figure1}}, we see that the EM estimation is superior to that of the approximate MLE and moment estimators.
As seen from the right panel, this superiority comes at a computational cost rapidly increasing in $p$.
