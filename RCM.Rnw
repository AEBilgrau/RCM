...
The model was fitted using three different initial sets of parameters which all converged to the same likelihood and parameter estimates.
The log-likelihood trace and computation times are seen in Figure \ref{\Sexpr{loglik.trace.plot}}.
...
We checked if the identified modules were prognostic for overall survival (OS) in the CHOP and R-CHOP-treated cohorts of the GSE10846 datasets.
...
Interestingly, the gene with highest connectivity in this module is
...
is number 16 further down the connectivity list as compelling research has shown that the S100 family of calcium-binding proteins maintain immunosuppressive MDS cells at the tumor site \citep{Fulmer2008}.
