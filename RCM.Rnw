...
In high-dimensional datasets the number of features $p$ often far exceeds the number of samples $n$.
Since the number of parameters increases quadratically in $p$ and the sample covariance matrix becomes singular when $p > n$ many shrinkage and regularization estimators have been proposed to combat the accompanying problems by effectively increasing the degrees of freedom. These examples include the graphical LASSO and ridge estimation of the precision matrix \citep{Friedman2008, vanWieringen2014}.
...
\noindent To evaluate $P(Y = y | \vX = \vx)$ in cases where the sample covariance matrix is not invertible, a small constant was added to the diagonal to allow for stable inversion similar to \citet{Friedman1989}. In the given setup, this modification was only necessary for QDA in the low-dimensional cases.
