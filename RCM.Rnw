...
We wished to indentify and examine gene-gene interactions in gene expression data of 89 diffuse large B-cell lymphoma samples from our own lab \citep{DybkaerBoegsted2015} as others \citep{Agnelli2011, Clarke2013} have successfully applied network analysis in other malignancies.
...
Most methods for analysing gene-gene interactions available, however, have one or more of several shortcomings.
Particularly as with our case, the methods are quickly become unstable or even unapplicable when the sample size is small or moderate.
...
We do so by utilizing a large number of genomic diffuse large B-cell lymphoma (DLBCL) datasets which, as with all major groups of cancer, are now publicly available online.
