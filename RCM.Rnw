...
\begin{abstract}
Estimating gene networks based on data of diseased tissue is one of the major challenges of cancer systems biology as it allows us to improve our understanding of disease pathology and eventually identify new drug targets.
Motivated by the need for improving the inherently unstable covariance estimation methods compounded by noisy gene expression data, we present a hierarchical random covariance model applied as a meta analysis of gene networks across multiple large-scale gene expression studies of diffuse large B-cell lymphoma (DLBCL).
