...
Genes and their products carry out their functions in concerted effort in intricate networks.
Network analysis of genomic data has consequently been an inescapable success in elucidating gene interactions through many different approaches.
For the same reasons, we wished to apply network reconstruction to gene expression data from diffuse large B-cell lymphoma samples from our own lab \citet{DybkaerBoegsted2015}.
However, current methods has one or more of several shortcomings.

Crudely, many network analysis methods---such as relevance or correlation networks \citep{Horvath2011}, Gaussian graphical models \citep{}, and Bayesian networks \citep{}---can be categorized according to their use of marginal, partial, or semi-partial correlations.
The underlying assumption of all is that correlation hints at causation.
Different correlation measures then separate the methods in the attempt to remove the influence of other genes.
See \citep{Markowetz2007a} for an overview.
