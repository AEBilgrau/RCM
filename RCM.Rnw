...
\begin{abstract}
Estimating gene networks in combination with posthoc analysis based on data from malignant tissue is a major challenge in cancer systems biology which improves our understanding of disease pathology leading to identification of new drug targets.
We present a hierarchical random covariance model applied as a meta-analysis of gene networks across 11 large-scale gene expression studies of diffuse large B-cell lymphoma (DLBCL).
The approach is motivated by the need for improving the inherently unstable covariance estimation compounded by noisy gene expression data and inspired by traditional meta-analysis using random effects models.
We derive and compare basic properties and estimators of the model.
...
As such, it provides a basis for meta- or integrative analysis of covariance matrices where the classes are formed by studies.
In a posthoc analysis of the estimated common covariance matrix for the DLBCL data we were able to identify biologically meaningful gene networks of prognostic value.
Of particular interest was the identification of a network with the S100 family of calcium-binding proteins as central players which further fuels the indications that knockdown of these proteins may improve the immunotherapy strategies and outcome of lymphoma patients.
\end{abstract}
...
Analysis of genomic data has consequently been an inescapable success in elucidating gene interactions and functions through many different approaches.
Notably, \citep{Agnelli2011, Clarke2013} have successfully applied network analysis in other malignancies to learn novel gene-gene interactions.
We interrogated and examined gene-gene interactions in gene expression data of 89 diffuse large B-cell lymphoma (DLBCL) samples from our own lab \citep{DybkaerBoegsted2015} data together with 10 other large-scale DLBCL studies to identify new and corroborate known functional gene-gene relationships.
Analysing gene-gene interactions using correlations is a simple and well-tested method \citep{Horvath2011} relying on guilt-by-association.
Most methods for analysing gene-gene interactions available, however, often have several shortcomings.
Particularly as with our case, the methods quickly become unstable or even inapplicable when the sample size is small or moderate.
Therefore we incorporated the information form multiple other publicly available gene expression DLBCL datasets to obtain a more stable estimate of the gene-gene interactions.

Crudely, many network analysis methods---such as correlation networks \citep{Horvath2011}, Gaussian graphical models \citep{Lauritzen1996}, and Bayesian networks---may be categorized according to their use of marginal, partial, or semi-partial correlations.
The underlying assumption of them all is that correlation hints at causation.
The different measures of correlation separate the methods by different attempts to remove the influence of remaining other genes \citep{Markowetz2007a}.
As the covariance matrix and its inverse holds this information, its estimation is a central topic to network analysis.
However, this fundamental problem of accurately estimating the covariance matrix (or its inverse, the precision matrix) is notoriously difficult.
...
Among this expanding list of applications is also an increasing number of high-dimensional applications and datasets publicly available in online repositories.

In genomic studies, the number of genes $p$ often far exceeds the number of samples $n$.
...
These ``single study'' estimators in combination with batch-effect removing methods \citep{Lee2014} could serve as a basis for analysing a common covariance matrix from multiple studies.
Table \ref{tab:studies} provides an overview of the studies used.
...
We do so by utilizing 10 genomic DLBCL datasets which, as with all major groups of cancer, are now publicly available online.
We use these studies in combination with data from our own laboratory as a network meta-analysis to arrive at a better estimate of the covariance matrix whilst accounting for and assessing inter-study variation.



% R> Hmisc::latex(DLBCLdata::DLBCL_overview[,-c(3,6)],
%             file = "",
%             label = "tab:studies",
%             title = "",
%             caption = "Overview of studies used.")
% R> t(dlbcl.dims)
\begin{table}[!tbp]
...
Hence, if $k$ classes are available with sample sizes $n_1, \ldots, n_k$, we consider the case where $p < \sum_{i=1}^k n_i$ while allowing $p$ to exceed $n_i$ for each individual class $i$.
In this paper, the $k$ classes correspond to $k$ genomic studies.

In Section \ref{sec:RCMmodel}, we propose the model for a common covariance matrix across multiple studies, derive estimators thereof, and propose an inter-study homogeneity measure to aid in assessing the variation between studies.
Next, we apply the model in Section \ref{sec:DLBCL} to $2{,}046$ DLBCL samples across 11 datasets.
We then assess the proposed model estimators via synthetic data in Section \ref{sec:estimationassessment} before concluding the manuscript in Section \ref{sec:conclusion}.
...
We model an observation $\vx_i$ from the $i$'th study as a $p$-dimensional zero-mean multivariate Gaussian vector with covariance matrix realized from an inverse Wishart distribution, i.e.\ $\vx_i$ follows the hierarchical model
\begin{align}
\begin{split} \label{eq:RCM}
  \vSigma_i  &\sim \calW^{-1}_p\big(\vPsi, \nu\big), \\
  \vx_i | \vSigma_i &\sim \calN_p(\vec{0}_p, \vSigma_i), \qquad i = 1, ..., k,
\end{split}
\end{align}
...
The ICC might in this sense be utilized in better quantifying the reproducibility of the covariance across studies.
...
Naturally, the ICC depends only on $\nu$.
...
To reproduce this document see \ref{suppA} or \url{http://github.com/AEBilgrau/RCM}.
...
The downloaded data together with a dataset from our own laboratory (GSE56315, \cite{DybkaerBoegsted2015}) yields a total of \Sexpr{rowSums(dlbcl.dims)["Samples"]} samples with study sizes in the range \Sexpr{paste(range(dlbcl.dims["Samples",]), collapse = "-")}.
The summarization using brainarray CDFs to Ensembl gene identifiers facilitates cross-platform integration.

After RMA normalization and summarization, the data was brought to a common scale by quantile normalizing all data to the common cumulative distribution function of all arrays.
...
Log-likelihood traces, iterations used, and computation times are seen in Figure \ref{\Sexpr{loglik.trace.plot}}.
...
\fig{\Sexpr{loglik.trace.plot}}{0.8\textwidth}{The trace of the log-likelihood for three different starting values of $\vPsi$ and $\nu$ using the EM algorithm and computational times in minutes. The number of iterations used for each fit is shown above.}

The estimated $\hat{\nu}$ yields a surprisingly low estimated ICC of $\Sexpr{round(get.ICC(dlbcl.rcm), 4)}$.
...
Bottom: A Hierarchical edge bundling representation of the network \citep{Holten2006, HierarchicalEdgeBundles} where edges loosely follow the dendrogram. Edge colors follow the color key. Only edges with a weight numerically larger than \Sexpr{round(dlbcl.par$threshold, 3)} are plotted.
}

Next, we outline one of many possible downstream analyses of the estimated covariance.
For simplicity we employed a standard correlation network analysis to the estimated common correlation matrix $\hat{\vec{R}}$ across all studies.
...
The tree was arbitrarily pruned at a height which produced \Sexpr{length(unique(dlbcl.modules))} modules named by colors.
Figure \ref{\Sexpr{dlbcl_plot}} shows these results.
A heat-map, the hierarchical tree, and the identified modules are seen at the top left.
...
As seen e.g.\ in the \Sexpr{cleanName(num2col[2])} module, genes from the same gene family cluster together.

To further investigate the low ICC, we refitted the RCM on the subset of the \Sexpr{length(the.genes)} genes in the \Sexpr{cleanName(the.module)} module.
This yielded an
$\text{ICC}(\Sexpr{round(the.rcm$nu, 2)}) = \Sexpr{round(get.ICC(the.rcm),3)}$.
...
We checked if the identified modules were prognostic for overall survival (OS) in the CHOP and R-CHOP-treated cohort datasets of GSE10846.
...
The module eigengene is the first principal component of the expression matrix of the module which thus can be represented by a linear combination of the module genes.
...
The top row shows $95\%$ and $99\%$ CI for the hazard ratio for each eigengene in the multiple Cox proportional hazards model containing all eigengenes.
The bottom row shows Kaplan-Meier estimates (and $95\%$ CI) for the overall survival for patients stratified by the dichotomized \Sexpr{cleanName(the.module)} eigengene.
}

From the survival analysis, the \Sexpr{cleanName(the.other.module)} module appeared particularly interesting since it identifies a group of patients with high value of the corresponding eigengene and poor survival, thus making them candidates for experimental therapies.
The gene with highest connectivity in this module is
\Sexpr{dlbcl.mod.tab.genes.copy[1, the.other.module]} (MRP8; calgranulin A) and the gene
\Sexpr{dlbcl.mod.tab.genes.copy[16, the.other.module]} (MRP14; calgranulin B)
appears as number 16 further down the connectivity list.
This is interesting as compelling research has shown that the S100 family of calcium-binding proteins maintain immunosuppressive MDS cells at the tumor site \citep{Fulmer2008}.
Notably, in mice injected with lymphoma cells, knockout of S100A9 resulted in greater tumor infiltration of T-cells and less accumulation of MDS cells than that seen in wild-type mice \citep{Cheng2008}.
The knockout mice had higher rates of tumor rejection and lower tumor size than their wild-type littermates.
This result indicates that knockdown of these proteins may improve the outcome of immunotherapy strategies in patients with values of the eigengene of the \Sexpr{cleanName(the.other.module)} module.

The \Sexpr{cleanName(the.module)} module also appeared interesting as it marks a gene cluster identifying DLBCL patients with significantly improved outcome.
...
CHIT3L1 (also known as YKL40) has been linked to the AKT anti-apoptotic signaling pathway in glioblastoma \citep{Francescone2011} and thus high CHIT3L1 is associated with poor outcome.
...
<<large_p_dblcl_analysis, echo=FALSE, results='hide'>>=
@

We finally investigated the choice of using the top \Sexpr{dlbcl.par$top.n} most variable genes and fitted the RCM with the top \Sexpr{dlbcl.par2$top.n} most variable gene.
The approximate MLE algorithm followed by the EM algorithm estimated an $\Sexpr{dlbcl.par2$top.n} \times \Sexpr{dlbcl.par2$top.n}$ matrix $\hvPsi$ and $\hat{\nu} = \Sexpr{paste(round(dlbcl.rcm2$nu, 1))}$ in $\Sexpr{round(dlbcl.rcm2$time[3]/3600, 1)}$ hours.
This implies that
$\text{ICC}(\hat{\nu}) = \Sexpr{round(get.ICC(dlbcl.rcm2),6)}$; a somewhat smaller value than the above.
The common correlation matrix was computed from $\hvPsi$ and $\hat{\nu}$ and hierarchical clustering was applied as above.
This showed that the same genes in the top \Sexpr{dlbcl.par$top.n} based analysis also largely cluster together in this clustering.
Particularly, all but one gene from the \Sexpr{cleanName(the.other.module)} module cluster together.
...
To assess the precision and stability of the estimation procedure we generated data from the hierarchical model \eqref{eq:RCM} under two scenarios with $k = \Sexpr{par.ni.sp[["k"]]}$ studies with an equal number of observations, $n_1 = n_2 = n_3$, and with different choices of the number of genes $p$.
In the first scenario, we set $p = \Sexpr{par.ni.sp[["p"]]}$ with the number of observations $n_i$ in each study varying in the range
$\Sexpr{paste0("[", min(par.ni.sp[["n.obs"]]), ",", max(par.ni.sp[["n.obs"]]), "]")}$.
In the second scenario, we set $p = \Sexpr{par.ni.lp[["p"]]}$ with $n_i$ in the range
$\Sexpr{paste0("[", min(par.ni.lp[["n.obs"]]), ",", max(par.ni.lp[["n.obs"]]), "]")}$.
In both scenarios, we chose the parameters $\nu = \Sexpr{par.ni.sp[["nu"]]}$ and $\Psi_{ij} = 1$ for all $i = j$ and $\Psi_{ij} = 0.5$ for all $i \neq j$.
...
In both scenarios, the weighted sum of squared errors $\text{SSE}(\hvSigma)$ for each $n_i$ and estimator was computed for $\Sexpr{par.ni.sp[["n.sims"]]}$ datasets.
The mean SSEs (and 99\% CI) of these values are seen in panels A and B of Figure \ref{\Sexpr{plot.sim.study}} as function of $n_i$.

In scenario 1, the EM approach used a total of \Sexpr{sim.est.time["time.em.elapsed"]} minutes of computation time for $\Sexpr{par.ni.sp$n.sims}$ fits compared to \Sexpr{sim.est.time["time.pool.elapsed"]} and \Sexpr{sim.est.time["time.mle.elapsed"]} minutes for the pool and approximate MLE approach, respectively.
In scenario 2, the EM, pooled, and approximate MLE approach used
\Sexpr{sim.est.time.l["time.em.elapsed"]},
\Sexpr{sim.est.time.l["time.pool.elapsed"]}, and
\Sexpr{sim.est.time.l["time.mle.elapsed"]} minutes respectively.
Panel C of Figure \ref{\Sexpr{plot.sim.study}} shows the computational time in a third scenario for more extreme values of $p$.

\fig{\Sexpr{plot.sim.study}}{\textwidth}{
Panel A: The mean SSE (and 99 \% CI) for scenario 1 from \Sexpr{par.ni.sp$n.sims} simulations as a function of the number of samples $n_i$ in each of \Sexpr{par.ni.sp$k} studies.
Panel B: The mean SSE (and 99 \% CI) for scenario 2.
Panel C: The mean computation time of $\Sexpr{par.t$n.sims}$ fits with varying dimension $p$.
All panels show the constant parameters in the scenario.
}

From both panels A and B of Figure \ref{\Sexpr{plot.sim.study}}, we see that the EM estimation is superior with less variation to that of the approximate MLE and moment estimators.
As seen from the panel C and above computational times, this superiority comes at a computational cost rapidly increasing in $p$.
...
This article provides a basic framework for modeling a common covariance structure across multiple studies applied to identify biologically meaningful gene-gene interaction networks in DLBCL.
...
The model-based approach allows investigating and testing both pairwise and global covariance structures correlations of genes, which we consider for future research.
Another feature is the invertible estimate provided by the RCM, which allows users to explore the corresponding Gaussian graphical model.
Also desirable is the explicit and interpretable quantification of the inter-study variance.
If $\nu$ is estimated to be large, the studies exhibit a largely common covariance structure, and vice-versa when $\nu$ is estimated to be small.
...
However, we believe this work could be further enriched by combining the method with regularized estimation.
Such a generalization of the model to $p \gg n_\bullet$ is extremely interesting though out of scope for this article.
...
For example, while non-zero entries can often be accurately recalled in graphical lasso, actual estimates of the covariances (or precisions) can still be heavily biased.
Large sample-sizes are still needed to achieve unbiased estimates of the covariance due to the bias-variance trade-off.
An increased focus should therefore also appointed to efficiently aggregating datasets and achieving sufficiently large sample sizes to allow for stable and unbiased estimation of covariance matrices.
Likewise, results should be validated in independent experiments.
...
\begin{supplement}
\sname{Supplement C}\label{suppC}
\stitle{Appendices}
\slink[url]{http://people.math.aau.dk/\~{}abilgrau/RCM/SuppC/}
\sdescription{Appendices for the article.}
\end{supplement}
