...
We similarly interrogated and examined gene-gene interactions in gene expression data of 89 diffuse large B-cell lymphoma (DLBCL) samples from our own lab \citep{DybkaerBoegsted2015} data to identify new and corroborate known functional gene-gene relationships.
...
Particularly as with our case, the methods quickly become unstable or even unapplicable when the sample size is small or moderate.
Therefore we incorrporated the information form multiple other publicly available gene expression DLBCL datasets to obtain a more stable estimate of the gene-gene interactions.
