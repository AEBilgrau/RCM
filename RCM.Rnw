...
Bottom: A Hierarchical edge bundling representation of the network \citep{Holten2006} where edges loosely follow the dendrogram. Edge colors follow the color key. Only edges with a weight numerically larger than \Sexpr{round(dlbcl.par$threshold, 3)} are plotted.
}
