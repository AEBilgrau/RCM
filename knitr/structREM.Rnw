...
It is applicable where the groups are believed to share a common covariance of interest obscured by noise.
...
We derive the basic properties and estimators of the model and compare our estimators method to the straight-forward approaches of simple averages or ``mega analysis'' were the datasets are combined, reprocessed, and plugged into the usual estimators.
...
The usual bias-corrected maximum likelihood estimator (MLE), the sample covariance matrix, have long been known to perform poorly in general due to high variability \citep{Dempster1972}.
The sample covariance is singular when the sample size $n$ is less than the number of variables $p$ and increasingly ill-conditioned as $p$ approaches $n$.
Because of its central statistical role the list of statistical methods and applications utilizing the estimated covariance matrix is extraordinarily long.
Beside the many standard statistical methods such as principal component analysis (PCA), linear discriminant analysis (LDA), quadratic discriminant analysis (QDA), more direct applications include [examples in medicine, biology, genetics, finance, forensics, physics, engineering, signal processing].
...
Regular meta-analysis comes principally in two difference flavors, fixed and random, corresponding to the assumption on the nature of the effects of a treatment across studies, say.
Random-effects meta-analysis models the effects as random variables realized for each study.
...
Hence, a graphical analog to \citet{Choi2003} is the following relatively simple graphical random effects model (GREM) of the observations.
Let $p$ be the number of features and $k$ the number of studies.
We model the each sample for the $i$th study as a $p$-dimensional zero-mean multivariate gaussian vector with the covariance matrix realized from a inverse Wishart distribution, i.e.\ the hierarchical model,
\begin{align}
...
While we are not able to shown general log-concavity of the log-likelihood, we show that it is log-concave in $\nu$ for fixed $\vSigma$, cf.\ Appendix \ref{sec:concaveloglik}, and hence there exists a unique maxima for the marginal $\ell(\nu)$.
Reversely, for fixed $\nu$ we show that the hessian in any stationary point (where $\frac{\partial\ell}{\partial\vPsi} = 0$) is negative semi-definite, cf.\ Appendix \ref{sec:negativedefinite}, and hence a local maxima. This combined with the observation that $\ell(\vPsi) \to -\infty$ whenever an eigenvalue $\lambda_i(\vPsi) \to 0$, we conjecture that any stationary point indeed a global maximum.
...
\end{algorithmic}
\end{algorithm}
The update function $U$ in the algorithm is defined by the derived estimators above. That is, equations \eqref{eq:moment}, \eqref{eq:mle}, or \eqref{eq:em} define $U$ to be the moment, approximate MLE, and EM estimate respectively.

This algorithm utilize the results about the log-likelihood and thus provide us with a guarantee of convergence along with the advantage of a very simple implementation. However, the obvious disadvantage is, that the idenified maxima might be a saddle-point when considering the log-likelihood jointly in $(\vPsi, \nu)$.
...
\vPsi =
  ...
  \end{bmatrix}.
\end{align*}
The number of observations is each study $n$ was varied range $\Sexpr{paste0("[", min(par.ne$n.obs), ",", max(par.ne$n.obs), "]")}$.

We measure the precision of the estimates values against the expected covariance matrix given by \eqref{eq:expcovar}.
Let $\hvPsi$ and $\hat{\nu}$ be the estimates obtained using the moment, EM, or approximate MLE in the model described. We then use the plug-in estimate
\begin{align}
  \label{eq:sigma_plugin}
  \hat{\vSigma}_\text{GREM} = \frac{1}{\hat{\nu} - p - 1}\hvPsi
\end{align}
of $\vSigma$. We benchmark the three proposed methods against the pooled covariance matrix,
\begin{align}
  \label{eq:pooled}
  \hvSigma_\text{pool}
...
The benchmarking measure used is the following sum of squared errors,
\begin{align*}
...
For each $n$ from $\Sexpr{min(par.ne$n.obs)}$ to $\Sexpr{max(par.ne$n.obs)}$, the squared sum of errors for each estimator, $\text{SSE}(\hvSigma_\text{GREM})$ and $\text{SSE}(\hvSigma_\text{pool})$, where computed for $\Sexpr{par.ne[["n.sims"]]}$ datasets and the average of these values seen in figure \ref{fig:numerical_experiment_plot} as function of $n$.
<<numerical_experiment_plot, echo=FALSE, results='markup', fig.pos="htb", fig.height=7, fig.width=7, out.width = ".8\\linewidth", fig.scap = "", fig.cap=paste0("The average sum or squared errors (SSE), of ", par.ne$n.sims, " simulations, as a function of the number of samples $n_i$ in each study.")>>=
@
Unsurprisingly, the model can outperform the simple average covariance matrix.
We see that the EM estimation is the superior to that of the approximate MLE and moment.
Also seen is that the moment estimate yield the exact same results as using the pooled average, which could be expected by looking at \eqref{eq:moment}, \eqref{eq:sigma_plugin}, and \eqref{eq:pooled}.

The algorithm along with the different estimators were implemented in the statistical programming language R \cite{R}.
...
An quite different application of the GREM estimate can be found as a novel version of discriminant analysis.
As seen below, the estimates obtained can be utilized in supervised learning as a intermediate case of linear discriminant analysis (LDA) and quadratic discriminant analysis (QDA).

Suppose $Y$ is a random variable denoting the class $1, ..., k$ and suppose $\vx$ is a random vector of the explanatory variables. Recall that QDA (and LDA) finds the class $y$ maximizing
\begin{align*}
  P(Y = y | \vX = \vx) =
    \frac{\pi_y f(\vx | Y = y)}
         {\sum_{y' = 1}^k \pi_{y'} f(\vx | Y = y')}
\end{align*}
where $\vx | Y = y$ is assumed multivariate gaussian, i.e.\
\begin{align*}
...
An intermediate classifier of LDA and QDA can thus be constructed by assuming the $\vSigma_y$'s inversely Wishart distributed as in \eqref{eq:REM}, i.e., $\vSigma_y \sim \calW^{-1}(\vPsi, \nu)$, and using the estimates of a common $\vSigma$ discussed above.
This hierarchical discriminant analysis (HDA), for lack of a better name, is thus straight-forward to implement given that
\begin{align*}
f(\vx | Y = y)
...
Note also that HDA thus also generalizes the LDA in some sense to have multivariate $t$-like distribution.
...
First, class labels was generated from a multinomial distribution with equal probabilities for each class, $\pi_1 = \pi_2 = \pi_3 = \frac{1}{3}$.
Hence, in each simulation round $\Sexpr{round(par.xda[["n.obs"]]/par.xda[["K"]], 2)}$ observations was expected.
Conditional on the class label the observations where drawn i.i.d.\ from a multivariate gaussian distribution, i.e.\ for the $i$th observation, $x_{i}|K = k \sim \calN_p(\vmu_k, \vSigma_k)$.
...
The \Sexpr{par.xda[["K"]]} covariance matrices were chosen to either be (1) equal and spherical, (2) unqual and spherical, (3) equal and highly elliptical, and (4) unequal and highly elliptical.
In scenario (1), $\vSigma_1 = \vSigma_2 = \vSigma_3 = \vI$.
In scenario (2), $\vSigma_k = k \vI$.
In scenario (3), the covariance matrices equal, $\vSigma_1 = \vSigma_2 = \vSigma_3$, and chosen such that the squareroot of the $d$ eigenvalues are chosen equidistantly on the interval 10 to 1 and a randomly (uniformly) oriented orthonormal basis is chosen and used for all components \citep{Friedman1989}.
In scenario (4), the eigenvalues are chosen as in scenario (3) for all $\vSigma_k$ but the orientation of the orthonormal basis of eigenvectors differs.

In all scenarios expect (3) the mean values were chosen to $\vec{\mu}_1 = \vec{0}, \vec{\mu}_2 = 3\vec{e}_1, \vec{\mu}_3 = 4\vec{e}_2$ where $\vec{e}_j$ denotes the $j$th basis-vector.
In scenario (3), $\vec{\mu}_1 = \vec{0}$ and the remaning means values are chosen such that the differences project main onto the low-variance subspace

Each of the four scnarios where probed with varying dimensionality $p = \Sexpr{paste(par.xda[["p.dims"]], collapse = ", ")}$.
...
In the equal and spherical case, HDA yields almost identical results to LDA which unsurprisingly outperformes QDA.
Also perhaps expected, the difference between LDA and QDA is less prominent for low dimensional spaces.
In the unqual and spherical case, HDA insubstantially outperforms LDA which both outperform QDA.
In the highest dimensional case where $n$ is of the same size as $p$, HDA is substatially better than the alternatives.


Most interestingly, HDA is seen to always perform at least as good as LDA in all scenarios for all practical purposes.

The above demonstrate that HDA as potentially useful addition to the descriminant analysis toolbox.
...
\end{align*}
We draw this derivative with $k = 1$ and other appropriately chosen parameters as seen in Figure \ref{fig:one_dimensional_loglik}
<<one_dimensional_loglik, echo = TRUE, fig.height=3, fig.width=3, fig.pos="htb", ig.scap = "", fig.cap = "One-dimensional case of the derivative of log-likelihood.">>=
@
...
<<log_gamma_ratio, echo=-4, fig.height=4.5/2, fig.width=4.5, fig.pos="ht", fig.scap = "", fig.cap="Plots of the mapping given in \\eqref{eq:logGammaRatio} for diffent values of $a$.">>=
...
\begin{proposition}[Unique maximum]
...
The proposition follows straight-forwardly from Lemma \ref{lem:elltominusinfty} and
