...
<<numerical_experiment_plot, echo=FALSE, results='markup', fig.pos="htb", fig.height=7, fig.width=7*2, out.width = ".8\\linewidth", fig.scap = "", fig.cap=paste0("The average sum of squared errors (SSE), of ", par.ne$n.sims, " simulations, as a function of the number of samples $n_i$ in each study.")>>=
@
...
<<one_dimensional_loglik, echo=-3, fig.height=3, fig.width=6, fig.pos="htb", ig.scap = "", fig.cap = "One-dimensional log-likelihood and its the derivative.">>=
@
...
<<log_gamma_ratio, echo=-3, fig.height=4.5/2, fig.width=4.5, fig.pos="ht", fig.scap = "", fig.cap="Plots of the mapping given in \\eqref{eq:logGammaRatio} for diffent values of $a$.">>=
@
\end{proof}
...
If $\nu$ is fixed and $\sum_{a=1}^k x_a \geq p$ then the log-likelihood $\ell(\vPsi) \to -\infty$ whenever $\exists i : \lambda_i \to 0$, where $\lambda_i$ is a eigenvalue of $\vPsi$.
\end{lemma}
