...
The corresponding GEO-accession numbers are \Sexpr{paste(grep("XXX", unique(studies$GSE), invert = TRUE, value = TRUE), collapse = ", ")} of various microarray platforms.
...
<<dlbcl_plot_1, echo = FALSE, fig.height=7/2.5, fig.width=7, fig.pos="htb", fig.scap = "", fig.cap="\\it Distribution of the correlation coefficents given by the entries of $\\vec{\\hat{R}}$.">>=
@

<<dlbcl_mappings, echo=FALSE, results='hide'>>=
@

<<dlbcl_plot_2, echo = FALSE, message=FALSE, results='hide', dev='png', fig.ext='png', fig.width=1500, fig.height=2300, dpi=150, fig.pos="htb", fig.scap="", fig.cap="\\it{}Top left: dendrograms of the hierachical clustering of the TOM-distance measure, the modules based on the clustering, and the correlation matrix. Top right: a graphical representation of the correlation matrix as laid out by the Fruchterman-Reingold force-based algorithm using the edge-weights. The nodes are coloured after the identified modules with sizes proportional to their connectivity. The opacity of the edges are drawn as function of the edge weight. Bottom: Hieriachical edge bundeling representation of the correlation network graph. Negative and postive correlation are coloured blue and red, respectively. Again, the strength of the correlation determine the transparency of the edge.">>=
@
...
One might question the added ultiliy of the $\nu$ parameter is an obvious critique of the GREM.
