...
With the GREM framework, we show that, properly scaled, the pooled covariance and mean covariance matrices can be viewed as different moment estimators.
...
Both estimates of $\vPsi$ are seen to be unbiased.\fxnote{Efficientcy and consistency?}
...
This coordinate ascent-like\fxnote{Or what is it?} approach is repeated until convergence.
...
Also seen is that the moment estimate yield the exact same results as using the mean, which could be expected by looking at \eqref{eq:meanestimate}, \eqref{eq:sigma_plugin}, and \eqref{eq:pooledest}.\fxnote{Is this surprising?}
...
A large amount of DLBCL datasets are now available online at the NCBI (National Center for Biotechnology Information) Gene Expression Omnibus (GEO) website. We downloaded 10 DLBCL gene expression studies and preprocessed using the custom brainarray chip definition files (CDF) and RMA-normalization [7, 3]. The corresponding GEO-accession numbers are \Sexpr{paste(grep("XXX", unique(studies$GSE), invert = TRUE, value = TRUE), collapse = ", ")} of various microarray platforms. The downloaded data together with our own yields a total of \Sexpr{rowSums(dlbcl.dims)["Samples"]} samples with studies sizes in the range \Sexpr{paste(range(dlbcl.dims["Samples",]), collapse = "-")}. The summarization using brainarray CDFs facilitates cross-platform integration.
...
The datasets were reduced to the total of \Sexpr{dim(gep[[1]])["Features"]} common features represented in all studies and array platforms.

\subsubsection{Graphical meta-analysis}
We wanted to perform coexpression network (i.e.\ correlation network) analysis across all datasets.
...
The parameters of the GREM model was estimated using the EM algorithm and yielded

<<Psi, echo=FALSE, fig.height=7/2, fig.width = 7>>=
r <- order(rowSums(abs(dlbcl.grem$Psi)))
img <- dlbcl.grem$Psi[r,r]
img <- img[ncol(img):1,]
par(mar = c(0,0,0,0))
graphics::image(img, axes = FALSE, asp = 1,
                col = heat.colors(20))
text(-0.05, 0.5, expression(hat(bold(Psi)) == phantom(a)))
@

\noindent and a $\hat{\nu} = \Sexpr{round(dlbcl.grem[["nu"]], 2)}$.\fxnote{Interpretation of $\nu$?}
...
<<dlbcl_plot_1, echo = FALSE, fig.height=7/3, fig.width=7, fig.pos="htb", fig.scap = "", fig.cap="\\it Distribution of the correlation coefficents given by the entries of $\\vec{\\hat{R}}$.">>=
@

<<dlbcl_plot_2, echo = FALSE, message=FALSE, results='hide', fig.height=7, fig.width=2*7, fig.pos="htb", fig.scap = "", fig.cap="\\it Left: dendrograms of the hierachical clustering of the TOM-distance measure, the modules based on the clustering, and the correlation matrix. Right: a graphical representation of the correlation matrix as laid out by the Fruchterman-Reingold force-based algorithm using the edge-weights. The nodes are coloured after the identified modules with sizes proportional to their connectivity. The opacity of the edges are drawn as function of the edge weight.">>=
@
...
To identify subgraphs, we used agglomerative hierachical clustering with \Sexpr{dlbcl.clust$method} linkage on the distance measure defined by topological overlap matrix (TOM) \citep{Horvath2011} using the WGCNA-package \citep{Langfelder2008}.
The tree was pruned at dynamic heights (using the function \texttt{cutreeDynamicTree}) which identified \Sexpr{length(unique(dlbcl.modules))} tightly connected subgraphs or modules coloured and named arbitrarily by colors.
Figure \ref{fig:dlbcl_plot_2} show these results.
...
@
An quite different application of the GREM estimate can be found as a novel discriminant analysis.
...
Note also that HDA thus also generalizes the LDA in some sense to have multivariate $t$-like distribution. Multivariate $t$-distributions has before considered for discriminant analysis, cf.\ \citet{Andrews2011}.
...
<<one_dimensional_loglik, echo=-3, fig.height=3, fig.width=6, fig.pos="htb", ig.scap = "", fig.cap = "\\it One-dimensional log-likelihood and its the derivative.">>=
@
