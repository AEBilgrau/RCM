...
%\title{A hierachical model for estimation of common covariance matrices from multiple studies}
\author{
...
The sample covariance become increasingly ill-conditioned as number of variables $p$ approaches the sample size $n$ and singular when $p$ exceeds $n$.
...
We model each sample $\vx$ of the $i$th study as a $p$-dimensional zero-mean multivariate gaussian vector with covariance matrix realized from an inverse Wishart distribution, i.e.\ $\vx$ follows the hierarchical model
\begin{align}
...
\end{align}
\normalsize
where constant terms are abbreviated by $c$ and $\Gamma_p$ is the multivariate generalization of the gamma function $\Gamma$ seen in Appendix \ref{sec:multigamma}.
...
Reversely, for fixed $\nu$ we show that the Hessian in any stationary point (i.e.\ where $\frac{\partial\ell}{\partial\vPsi} = \vec{0}$) is negative definite, cf.\ Lemma \ref{lem:negativesdefinite} in Appendix \ref{sec:negativedefinite}, and hence every stationary point is a local maxima.
...
We first attempted to derive MLE estimators using a first order approximation. However, as simulation experiments later shows, this estimator performed quite poorly and has therefore been deferred to Appendix \ref{sec:amle}.
...
Both estimates of $\vPsi$ are seen to be unbiased.
...
\begin{align*}
  H_0: \vSigma_1 = ... = \vSigma_k.
\end{align*}
The two are equivalent since the sampling of the covariance matrix from the inverse Wishart distribution become deterministic when $\nu = \infty$. Testing this hypothesis can also be interpreted as testing if the data is adequately explained when leaving out the hierarchical structure.

In practice, under $H_0$ or when $\nu$ is very large the estimated $\hat{\nu}_\text{obs}$ will be relatively small since the intra-study variance dominate the total variance.
The distribution of $\hat{\nu}$ under $H_0$ is not easily characterized.
However, the distribution of $\hat{\nu}$ can easily be approximated by simulation.
To do this, the model is simply fitted on datasets generated under $H_0$ a large number of times $N$ to get $\hat{\nu}_0^{(1)}, ..., \hat{\nu}_0^{(N)}$. An approximation of the $p$-value can then be obtained by \begin{align*}
  \frac{1}{N+1} \left( 1 + \sum_{j=1}^N \bbOne\!\big[\hat{\nu}_0^{(j)} < \hat{\nu}_\text{obs}\big] \right)
\end{align*}
where $\bbOne$ is the indicator function. Hence, this is approximately the fraction of $\hat{\nu}_0$'s smaller than $\hat{\nu}_\text{obs}$.
...
Also seen is that the moment estimate yield the exact same results as using the mean, which could be expected by looking at \eqref{eq:meanestimate}, \eqref{eq:sigma_plugin}, and \eqref{eq:pooledest}.
...
In all four scenarios, we generated a training dataset of a total of $n = \Sexpr{par.xda[["n.obs"]]}$ observations belonging to $k = \Sexpr{par.xda[["K"]]}$ classes.
...
This demonstrate that HDA as potentially useful addition to the discriminant analysis toolbox.
...
As demonstrated, combining multiple studies can yield a sufficiently large total sample size that allows for estimation of large covariance matrices without the use of regularization
...
The recent advances in such regularized techniques which allows for analysis for very large covariance matrices has unfortunately diminished the focus on collecting larger sample.
...
Large sample-sizes are still needed achieve unbiased estimates of the covariance.
...
One might question the added utility of the $\nu$ parameter is an obvious critique of the RCM.
For example, it is unclear how large a proportion a single extra parameter can explain of the inter-study variance.
...
\begin{lemma}
\label{lem:negativesdefinite}virtue
The hessian of the log-likelihood \eqref{eq:appendloglik} is negative definite in all stationary points for fixed $\nu$.
\end{lemma}
...
The second order derivative is can be thought of as a $\binom{p+1}{2} \times \binom{p+1}{2}$-dimensional matrix indexed by $(i,j)$ and $(k,l)$, $i \leq j$, $k \leq l$.
...
\begin{align}
  ...
  &= \sum_a (n_a + \nu)\tr\!\Big(\vY(\vI + \vS_a)\vY\Big).
  \label{eq:loglikequation}
\end{align}
We substitute \eqref{eq:loglikequation} into \eqref{eq:negativedefinte} to get
\begin{align*}
  &\sum_a (n_a + \nu)
  \tr\!\Big(\vY (\vI + \vS_a) \vY (\vI + \vS_a) - \vY (\vI + \vS_a) \vY \Big) \\
  &=  \sum_a (n_a + \nu)
  \tr\!\Big( \vY (\vI + \vS_a) \vY \big[ (\vI + \vS_a)  - \vI \big]\Big)
  < 0
\end{align*}
...
\begin{align*}
  \sum_a (n_a + \nu) \tr\!\Big(
    \vY(\vI + \vX_a\vX_a^\top)\vY\vX_a\big(\vI + \vX_a^\top\vX_a\big)^{-1}\vX_a^\top
  \Big)
  > 0.
\end{align*}
...
\subsection{Approximate maximum likelihood estimator}
\label{sec:amle}
...
%   \notag \\
  &= \frac{k\nu}{2} \vPsi^{-1}
    - \sum_{i=1}^k \frac{\nu + n_i}{2}
      ...
      \notag
\end{align}
While assuming $\vPsi$ unstructured is not strictly correct it (in this case) leads to the correct equation. The proper 1.\ order derivative can be seen in equation \eqref{eq:dloglik} and can easily be seen to be equivalent to the above.
