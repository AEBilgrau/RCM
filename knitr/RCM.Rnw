...
As such, it provides a basis for meta-analysis of covariance matrices where the classes are formed by datasets.
...
The results further corroborates that the identified modules are biologically meaningful and that RCM provides a reasonable estimate of the covariance.
