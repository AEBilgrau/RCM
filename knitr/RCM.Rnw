...
\title{Estimation of a common covariance matrix in multiple groups with applications in meta-analysis}
\author{
...
\begin{abstract}
We propose a hierarchical random effects model and estimators for a common covariance matrix in cases where multiple groups or datasets are present.
It is applicable where the datasets are believed to share a common covariance matrix of interest obscured by group-dependent noise.
...
% Though only a modest improvements, explicitly accounting for the inter-study variance seems superior to the alternative.
Simple inference of the introduced parameter measuring the intergroup homogeneity is suggested.
The model is also shown to be applicable as an intermediate between linear and quadratic discriminant analysis. \medskip

\noindent \textbf{Keywords:} \textit{meta-analysis, covariance estimation, integrative analysis, network integration, gaussian graphical modeling, discriminant analysis}
\end{abstract}
...
The sample covariance become increasingly ill-conditioned as the number of variables $p$ approaches the sample size $n$ and singular when $p$ exceeds $n$.
...
While the high-dimensional extension to $p > n$ is important it is out of scope here.
We restrict ourselves to the case where the total number of samples exceed $p$ the purposes of this paper.
Hence, if $k$ groups or datasets are available with sample sizes $n_1, ..., n_k$, we then consider the case where $p < \sum_{i=1}^k n_i$ while allowing $p > n_i$ for all or any $i$.

Although this work was motivated by gene-gene interaction networks in diffuse large B-cell lymphoma (DLBCL) where the covariance matrix contain all information about the conditional dependencies of the genes, the methods are general and not limited to such genomic data.
...
\section{A random effects model of the covariance}
The model below was motivated by regular meta-analysis.
Meta-analysis comes in various flavors corresponding to the assumption on the nature of the inter-study treatment effect.
Random-effects models (REM) for meta-analysis model the inter-study effects as random variables.
In a vein similar to the regular meta-analysis by \citet{Choi2003}, we think of the different studies as related but perturbed experiments.
Analogous to \citet{Choi2003}, we propose the following deceivingly simple random covariance model (RCM) of the observations.
...
\end{split}
\end{align}
where $\calN_p(\vec{\mu},\vSigma_i)$ denotes a $p$-dimensional multivariate gaussian distribution with mean $\vec{\mu}$, positive definite (pd) covariance matrix $\vSigma_i$, and probability density function (pdf)
\begin{align*}
...
\end{align}
where $\nu > p + 1$, and the common expected covariance matrix is
\begin{align}
  \label{eq:expcovar}
  \bbE[\vSigma_i] = \frac{\vPsi}{\nu-p-1} = \vSigma \quad\text{ for } \nu > p + 1.
\end{align}
Hence in the RCM given by \eqref{eq:RCM}, $\vSigma$ can be interpreted as a location-like parameter (though not strictly such) as it is the expected covariance matrix in each study.
The parameter $\nu$ inversely controls the inter-study or inter-group variation and can thus be considered an inter-study homogeneity parameter of the covariance structure.
A large $\nu$ corresponds to high study homogeneity (and thus low study heterogeneity) and vice versa for small $\nu$.
...
In the following, we wish to infer the parameters $\vSigma$ and $\nu$ from observed data.
...
The reparameterization is especially problematic (and indeed senseless) when the expected covariance cease to exists for $p - 1 < \nu \leq p + 1$.
Therefore, we use the usual parameterization $\vPsi$ in the actual fitting procedure and the and much of the remainder of this paper.
...
Suppose we have $n_i$ i.i.d.\ observations, $\vx_{i1}, \dots,\vx_{in_i}$, from each of the $i = 1,...,k$ independent studies from the model given in \eqref{eq:RCM}.
...
We show that it is log-concave in $\nu$ for fixed $\vPsi$, cf.\ Proposition \ref{prop:concavityinnu} in Appendix \ref{sec:concaveloglik}.
Reversely, for fixed $\nu$ and $\sum_{i = 1}^k n_i > p$ we show that the Hessian in any stationary point (i.e.\ where $\frac{\partial\ell}{\partial\vPsi} = \vec{0}$) is negative definite, cf.\ Lemma \ref{lem:negativesdefinite} in Appendix \ref{sec:negativedefinite}, and hence every stationary point is a local maxima.
...
% Note that if the common or expected covariance matrix is of interest, then the pooled covariance are obtained as estimators by plugging \eqref{eq:pooledest} into \eqref{eq:expcovar} which cancels out the scaling factor.
...
%   f(\vDelta_i)
%   &\propto
%        \big|(\nu - p - 1)^{-1}\vTheta\big|^{-\frac{\nu}{2}}
%        e^{-\frac{1}{2}\tr\big((\nu - p - 1)^{-1}\vTheta^{-1}\vDelta_i\big)}.
   \notag
\\
  \vS_i | \vDelta_i
...
\end{align*}
Hence, the expectation step is then given by
\begin{align*}
...
We denote the inverse of the estimate obtained by repeated iteration of \eqref{eq:em} with $\hvPsi_\text{EM}$.
...
The EM algorithm utilize the results about the log-likelihood and thus provide us with a guarantee of convergence along with the advantage of a very simple implementation.
...
\subsection[Interpretation and inference of nu]{Interpretation and inference of $\nu$}
This section describes basic inference on the parameter $\nu$.
By the RCM construction, $\nu$ parameterizes an inter-study variance where the size of $\nu$ corresponds the homogeneity between the studies or groups.
Large $\nu$ yield high study homogeneity and small $\nu$ yield low homogeneity.
Thus it is of interest to test if the estimated homogeneity $\hat{\nu}$ is extreme under the null-hypothesis of no heterogeneity (i.e.\ infinite homogeneity). We wish to test the hypothesis $H_0: \nu = \infty$ which is equivalent to
\begin{align*}
  H_0: \vSigma_1 = ... = \vSigma_k = \vSigma,
\end{align*}
The two are equivalent since the sampling of the covariance matrix from the inverse Wishart distribution become deterministic when $\nu = \infty$. Testing this hypothesis can therefore also be interpreted as testing if the data is adequately explained when leaving out the hierarchical structure.
...
However, the distribution of $\hat{\nu}$ under $H_0$ is not easily characterized. We propose to approximate the distribution of $\hat{\nu}$ under $H_0$ by simulation.
To do this, the model is simply fitted on datasets generated under $H_0$ a large number of times $N$ to get $\hat{\nu}_0^{(1)}, ..., \hat{\nu}_0^{(N)}$. As \textit{small} values of $\hat{\nu}$ are critical for $H_0$ and approximate acceptance regions can be constructed from $\hat{\nu}_0^{(j)}, j = 1,...,N$. Likewise, an approximation of a $P$-value testing $H_0$ can be obtained by
\begin{align*}
  P = \frac{1}{N+1} \left( 1 + \sum_{j=1}^N \bbOne\!\big[\hat{\nu}_0^{(j)} < \hat{\nu}_\text{obs}\big] \right)
\end{align*}
where $\bbOne[\cdot ]$ is the indicator function. Hence, this is approximately the fraction of $\hat{\nu}_0$'s smaller than $\hat{\nu}_\text{obs}$.


\subsubsection*{Intra-class correlation coefficient analogue}
We also introduce a descriptive statistic analogous to the intra-class correlation coefficient (ICC) value to better determine what constitute large values of $\nu$.
We wish to define a modified ICC (mICC) analogous to the ordinary ICC as the ratio of the between-study variation and the total variation of the samples.
...
To compute the mICC, we are thus interested in the ratio of the quantities $\var(\Sigma_{ij})$ and $\var(S_{ij})$ corresponding to the between-study and total variation of the covariance between $i$ and $j$, respectively.
That is, the mICC is the proportion of the total variance that is between studies,
\begin{align}
  \text{mICC}
  = \frac{\var(\Sigma_{ij})}{\var(S_{ij})}
  = \frac{\var(\Sigma_{ij})}{\var(\Sigma_{ij}) + \bbE[\var(S_{ij}|\vSigma)]},
  \label{eq:mICC}
\end{align}
where the second equality is obtained by the law of total variation and since $\bbE[S_{ij}|\vSigma] = \Sigma_{ij}$.
This equality agree with the usual ICC as $\bbE[\var(S_{ij}|\Sigma_{ij})]$ can be interpreted as the (expected) within-study variation.
Using the variance $\var(S_{ij}|\vSigma) = \Sigma_{ij}^2 + \Sigma_{ii} \Sigma_{jj}$ the needed quantities can be found
.
To compute an expression for \eqref{eq:mICC} we need to consider the third-order moments of the observations.
From the model,
\begin{align}
...
We continue with the conditional variance of $S_{ij}|\vSigma$ in the denominator of \eqref{eq:mICC},
\small
\begin{align}
...
% \begin{align}
%   &\var(n^{-1}S_{ij})\\
%   &= n^{-2}\Big( \var\!\big(\bbE[S_{ij}|\vSigma ]\big)
%     + \bbE\!\big[\var(S_{ij}|\vSigma )\big] \Big) \notag\\
% %  &= n^{-2}\Big( \var(\Sigma_{ij})
% %    + \bbE\big[ n( \Sigma_{ij}^2 + \Sigma_{ii}\Sigma_{jj} )\big] \Big)  \notag\\
%   &=  n^{-2}\Big( \var(\Sigma_{ij}) + n\bbE[\Sigma_{ij}^2]
%      + n\bbE[\Sigma_{ii}\Sigma_{jj}] \Big) \notag\\
% %   &= n^{-2}\Big( (n+1)\var(\Sigma_{ij}) + n\bbE[\Sigma_{ij}]^2
% %      + n\bbE[\Sigma_{ii}\Sigma_{jj}] \Big) \notag\\
%   &= n^{-2}\Big((n+1)\var(\Sigma_{ij}) + n\bbE[\Sigma_{ij}]^2
%     + n\cov(\Sigma_{ii}, \Sigma_{jj})
%     + n\bbE[\Sigma_{ii}]\bbE[\Sigma_{jj}] \Big) \notag\\
%   &= n^{-2}\Big( (n+1)\var(\Sigma_{ij}) + n\cov(\Sigma_{ii}, \Sigma_{jj})
%     + n(\nu-p-1)^{-2}(\Psi_{ij}^2 + \Psi_{ii}\Psi_{jj})
%      \Big)
%     \label{eq:varSij}
% \end{align}
% \normalsize
An expression of $\var(S_{ij})$ in terms of the elements of $\vPsi$ can then found by substituting of \eqref{eq:invwishcovar} and \eqref{eq:invwishvar} into \eqref{eq:varSij} and by extension an expression for the mICC \eqref{eq:mICC} can be obtained.
We omit this tedious calculation which can be verified to yield
\begin{align}
  \text{mICC}(\nu) &= \frac{1}{\nu-p},
  \label{eq:mICCexprs}
\end{align}
in the RCM.
Naturally enough, the mICC depends only on $\nu$.
A straight-forward plug-in estimator $\widehat{\text{mICC}}(\nu)$ of the mICC of some gene-gene interaction is then $\text{mICC}(\hat{\nu})$.
...
@
\section{Assessment of estimation procedures}
...
\vSigma =
  ...
  \end{bmatrix}.
\end{align*}
The number of observations is each study $n$ was varied in the range $\Sexpr{paste0("[", min(par.ne$n.obs), ",", max(par.ne$n.obs), "]")}$.

We measure the precision of the estimated values against the expected covariance matrix given by \eqref{eq:expcovar}.
Let $\hvPsi$ and $\hat{\nu}$ be the estimates obtained using the moment, EM, or approximate MLE (deferred to Appendix \ref{sec:amle}) approach as described. We benchmark the proposed estimators against the known truth. The benchmarking measure used is the following weighted sum of squared errors,
\begin{align*}
...
For each $n = \Sexpr{par.ne$n.obs}$, the squared sum of errors for each estimator, $\text{SSE}(\hvPsi)$, where computed for $\Sexpr{par.ne[["n.sims"]]}$ datasets and the average of these values are seen in Figure \ref{fig:numerical_experiment_plot} as function of the number of samples in each dataset $n_i$.

<<numerical_experiment_plot, echo=FALSE, results='markup', fig.pos="tb", fig.height=7/2, fig.width=7, out.width = ".8\\linewidth", fig.scap = "", fig.cap=paste0("\\it The estimated mean sum of squared errors (MSSE) based on ", par.ne$n.sims, " simulations, as a function of the number of samples $n_i$ in each study.")>>=
@

\noindent Unsurprisingly, the proper EM estimation outperform the simple alternative estimates.
We see that the EM estimation is superior to that of the approximate MLE and moment.
...
\subsection{Meta-analysis in DLBCL}
Diffuse large B-cell lymphoma (DLBCL) is an aggressive cancer subtype accounting for approximately $31 \%$ of all non-Hodgkin's lymphoma (NHL) \citep{Project1997} which itself constitute about $90 \%$ of all lymphomas.
%A further molecular sub-classification of DLBCL into germinal center B-cell-like (GCB) DBLCL and activated B-cell-like (ABC) DLCBL exists with very large differences at the molecular level.
%The GCB cases exhibit a clinically favorable overall survival prognosis compared to that of ABCs.
...
We downloaded 10 DLBCL large-scale gene expression studies and preprocessed these using the custom brainarray chip definition files (CDF) and RMA-normalization using the R-package \texttt{affy}. \citep{Dai2005, affy}
...
The downloaded data together with our own yields a total of \Sexpr{rowSums(dlbcl.dims)["Samples"]} samples with study sizes in the range \Sexpr{paste(range(dlbcl.dims["Samples",]), collapse = "-")}.
...
\fig{\Sexpr{dlbcl_plot_2}}{0.8\textwidth}{\it{}Top left: dendrograms of the hierarchical clustering, the identified modules, and the correlation matrix. Top right: the correlation network as laid out by the Fruchterman-Reingold algorithm using the edge-weights. The nodes are colored after the identified modules with sizes proportional to the node connectivity (defined by row sums of $\hat{\vec{R}}$). The opacity of the edges are drawn as function of the edge weight. Bottom: A Hierarchical edge bundling representation of the network. Negative and positive correlations are colored blue and red, resp.
}
...
The results further corroborates that the identified modules are biologically meaningful and that RCM provide a reasonable estimate of the covariance.
...
The above become a multivariate $t$-distribution when the sample sizes of each group is $1$.
Multivariate $t$-distributions has before been considered for discriminant analysis, cf.\ \citet{Andrews2011}.
...
In scenario (c), $\vec{\mu}_1 = \vec{0}$ and the remaining means values are chosen such that the differences project mainly onto the low-variance subspace

Using the described parameters a training and validation set each of $\Sexpr{par.xda[["n.obs"]]}$ and $\Sexpr{par.xda[["n.obs.valid"]]}$ observations, respectively, was generated.
...
The same holds true for the unequal and spherical case (b) and (c). Here HDA and LDA perform somewhat equally well, while HDA is
...
However, we recognize that further and more sophisticated simulation experiments are needed to explore scenarios where HDA should be considered a serious alternative.
...
This article provides a basic framework for modeling a common covariance structure across multiple groups or datasets.
...
For example, while non-zero entries often can be accurately recalled in graphical LASSO, actual estimates of the covariances (or precisions) can still be heavily biased. \cite{}
...
Hence, the present work should be considered a step in the direction of explicitly modeling the inter-study variation.
...
\begin{align*}
  ...
  \frac{\Gamma_p\left(\frac{\nu + n}{2}\right)}
       {\pi^\frac{np}{2}
          ...
          \Gamma_p\!\left(\frac{\nu}{2}\right)},
\end{align*}
which can help speed-up computations.
...
This section analyse the (non)-concavity of the log-likelihood function given in \eqref{eq:loglik}.
...
Alternatively, the first derivative not monotonically non-increasing which is a necessary and sufficient condition for a differentiable function of one variable to be concave.
\end{proof}
...
\section{Existence and uniqueness of likelihood maxima}
...
\end{align*}
is compact for any $\vPsi^*\succ 0$ since it is bounded and closed by the virtue of the Lemma and continuity of $\ell$, respectively.
The existence of a maximum follows from the extreme value theorem by the continuity of $\ell$. A stationary point exists due to Rolle's theorem and the differentiability of $\ell$.
...
\begin{align*}
   A_i = \Big\{\vPsi^* \in \calS_+ \Big| \vPsi(0) = \vPsi^*, \lim_{t->\infty} \vPsi(t) = m_i \Big\},
\end{align*}
are open sets in set of all postive semi-definite matrices $\calS_+$.
...
The matrix cookbook by \citet{Petersen2008} has been a useful reference here. See equations (41, p.\ 8) and (59, p.\ 9) and pages 14 and 52-53 in \citep{Petersen2008}. We first compute expression for the first and second order derivatives.
