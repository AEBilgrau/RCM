...
%   \notag \\
  &= \frac{k\nu}{2} \vPsi^{-1}
    - \sum_{i=1}^k \frac{\nu + n_i}{2}
      ...
      \notag
\end{align}
While assuming $\vPsi$ unstructured is not strictly correct it (in this case) leads to a valid equation. The proper 1.\ order derivative can be seen in equation \eqref{eq:dloglik} and can easily be seen to be equivalent to the above.
...
Note also that HDA generalizes the LDA in some sense to have a multivariate $t$-like distribution. Multivariate $t$-distributions has before been considered for discriminant analysis, cf.\ \citet{Andrews2011}. \fxnote{Elaboration on the multivariate $t$-distribution look and reference!}
