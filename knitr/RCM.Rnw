...
\begin{align*}
  (\vI + \vS_a)^{-1} - \vI = -\vX_a(\vI + \vX_a^\top\vX_a)^{-1}\vX_a^\top,
\end{align*}
whereby we need to show that
\begin{align*}
  \sum_a (n_a + \nu) \tr\!\Big(
  ...
  > 0.
\end{align*}
Since $n_a + \nu \geq 1$ and $\vI + \vX_a\vX_a^\top \geq \vI$, it is enough to shown
\begin{align*}
  \sum_a \tr\!\Big(
...
Then
$\big(\vI + \vX_a\vX_a^\top\big)^{-1} \geq (1 + \lambda_M)^{-1}\vI$
and we need to show
\begin{align*}
