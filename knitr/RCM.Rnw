...
\title{Estimation of covariance matrices from multiple studies}
\author{
...
\begin{abstract}
We propose a hierarchical model and estimators for a common covariance matrix in cases where multiple groups or datasets are present.
...
We derive the basic properties and estimators suggested by the model and compare their properties.
Though only a modest improvement, explicitly accounting for the inter-study variance seems superior to the alternative.
Simple inference on the introduced parameter measuring the intergroup homogeneity is also suggested.
...
Meta-analysis comes principally in various flavors corresponding to the assumption on the nature of the inter-study treatment effect.
...
The true covariance matrix is the effect of interest to be estimated in this paper.
Hence, following the train of thought of \citet{Choi2003} we propose the following simple random covariance model (RCM) of the observations.
...
\begin{split} \label{eq:REM}
  \vSigma_i  &\sim \calW^{-1}(\vPsi, \nu), \\
  \vx | \vSigma_i &\sim \calN_p(\vec{0}_p, \vSigma_i), \qquad i = 1, ..., k,
\end{split}
\end{align}
where $\calN_p(\vec{0}_p, \vSigma_i)$ denotes a multivariate gaussian distribution with probability density function (pdf)
\begin{align*}
  f(\vec{x} | \vSigma_i) =
  (2\pi)^{-\frac{p}{2}} \det(\vSigma_i)^{-\frac{1}{2}}
  e^{ -\frac{1}{2}\vx^\top \vSigma_i^{-1}\vx },
\end{align*}
and $\calW^{-1}(\vPsi, \nu)$ denotes an inverse Wishart distribution with pdf,
\begin{align*}
  f(\vSigma_i | \vPsi, \nu) =
  \frac{ |\vPsi|^\frac{\nu}{2} }{
...
\end{align*}
$\vPsi$ and $\vSigma_i$ are positive definite, $\nu > p - 1$, and $\Gamma_p$ is the multivariate generalization seen in Appendix \ref{sec:multigamma} of the gamma function $\Gamma$.
...
In the model, $\nu$ controls the inter-study variation and $(\nu - p - 1)\vSigma_i$ concentrates around $\vPsi$ for $\nu\to\infty$. I.e.\ the inter-study variation goes towards zero for larger $\nu$. We wish to infer the parameters $\vPsi$ and $\nu$ from the observed data so the common expected covariance matrix
\begin{align}
  \label{eq:expcovar}
...
Suppose we have $n_i$ i.i.d.\ observations, $\vx_{i1}, \dots,\vx_{in_i}$, from each of the $i = 1,...,k$ independent studies from the model given in \eqref{eq:REM}.
...
Note that $\vS_i$ is conditionally Wishart distributed, $\vS_i|\vSigma_i \sim \calW(\vPsi, n_i)$, by construction.
...
Reversely, for fixed $\nu$ and $n_\bullet > p$ we show that the Hessian in any stationary point (i.e.\ where $\frac{\partial\ell}{\partial\vPsi} = \vec{0}$) is negative definite, cf.\ Lemma \ref{lem:negativesdefinite} in Appendix \ref{sec:negativedefinite}, and hence every stationary point is a local maxima.
...
We first attempted to derive MLE estimators using a first order approximation. However, as simulation experiments later show, this estimator performed quite poorly and has therefore been deferred to Appendix \ref{sec:amle}.


\subsection{Moment estimator}
%In the following, estimation of $\vPsi$ for fixed $\nu$ is considered.
With the RCM framework, we show that, properly scaled, the pooled empirical covariance matrix and the mean empirical covariance matrix can be viewed as different moment estimators of $\vPsi$.
...
\end{align}
which is a scaled version of the pooled empirical covariance matrix.
...
From \eqref{eq:REM} we equivalently have that,
\begin{align*}
...
\end{align}
for $\vTheta$. The connection to the likelihood equation \eqref{eq:firstordderivloglik} is immediately seen. We denote the estimate obtained from \eqref{eq:em} with $\hvPsi_\text{EM}$.
...
That is, equations \eqref{eq:pooledest}, \eqref{eq:mle}, or \eqref{eq:em} define $U$ to be the moment, approximate MLE, and EM estimate respectively.
...
@
To assess the precision and stability of the estimation procedure we generated data from the hierarchical model \eqref{eq:REM} for $p = \Sexpr{par.ne[["p"]]}$ variables and $k = \Sexpr{par.ne[["k"]]}$ studies each with an equal number of observations, $n = n_1 = n_2 = n_3$. We chose the parameters $\nu = \Sexpr{par.ne[["nu"]]}$ and
\begin{align*}
\vPsi =
  \begin{bmatrix}
     1 & 0.5 & 0.5 & \cdots\\
...
\end{align}
of $\vSigma$. We benchmark the proposed estimators. The benchmarking measure used is the following weighted sum of squared errors,
\begin{align*}
...
For each $n$ from $\Sexpr{min(par.ne$n.obs)}$ to $\Sexpr{max(par.ne$n.obs)}$, the squared sum of errors for each estimator, $\text{SSE}(\hvSigma)$, where computed for $\Sexpr{par.ne[["n.sims"]]}$ datasets and the average of these values are seen in Figure \ref{fig:numerical_experiment_plot} as function of the number of samples in each dataset $n_i$.
<<numerical_experiment_plot, echo=FALSE, results='markup', fig.pos="tb", fig.height=7/2, fig.width=7, out.width = ".8\\linewidth", fig.scap = "", fig.cap=paste0("\\it The average sum of squared errors (SSE), of ", par.ne$n.sims, " simulations, as a function of the number of samples $n_i$ in each study.")>>=
@
...
Also seen is that the moment estimate yields the exact same results as using the mean, which could be expected by looking at \eqref{eq:sigma_plugin} and \eqref{eq:pooledest}.
...
@
Diffuse large B-cell lymphoma (DLBCL) is an aggressive cancer subtype of accounting for approximately $31 \%$ of all non-Hodgkin's lymphoma (NHL) \citep{Project1997} which itself constitute about $90 \%$ of all lymphomas.
...
We wanted to perform a co-expression network (i.e.\ weighted correlation network) analysis across all datasets.
...
The parameters of the RCM model was estimated using the EM algorithm and yielded the $\Sexpr{dlbcl.par$top.n} \times \Sexpr{dlbcl.par$top.n}$ matrix $\vPsi$ and $\hat{\nu} = \Sexpr{round(dlbcl.rcm[["nu"]], 2)}$.
...
Figure \ref{fig:dlbcl_plot_1} shows a histogram of the distribution of correlations.
...
@
Another application of the RCM is discriminant analysis.
...
f(\vx | Y = y)
  ...
  &= \frac{ |\vPsi|^{\frac{\nu}{2}} \Gamma_p\!\left(\frac{\nu + 1}{2}\right) }
          { \pi^{-\frac{n}{2}} |\vPsi + (\vx-\vec{\mu}_y)(\vx-\vec{\mu}_y)^\top|^{\frac{\nu + 1}{2}}
            \Gamma_p\!\left(\frac{\nu}{2}\right)},
\end{align*}
...
@
In order to be able to evaluate $P(Y = y | \vX = \vx)$ in cases where the sample covariance matrix is not invertible, a small constant was added to the diagonal to allow for stable inversion similar to \citet{Friedman1989}. In the given setup, this modification was only nessesary for QDA in the low-dimensional cases.
...
This sections shows the marginalization out of $\vSigma$ in \eqref{eq:loglik}. For ease of notation we drop the subscript $i$ used on $\vSigma_i$, $\vX_i$, $\vS_i = \vX_i \vX_i^\top$, and $n_i$ in the above text. We do the computation straight-forwardly by the assumptions of the model,
\small
\begin{align*}
  &f(\vX | \vPsi, \nu) \\
...
If $n_\bullet > p$ then the hessian of the log-likelihood \eqref{eq:appendloglik} is negative definite in all stationary points for fixed $\nu$.
\end{lemma}
...
Next, we prove the uniqueness of the maximum.
[...]
\end{proof}
...
\begin{proof}[Proof of Lemma \ref{lem:negativesdefinite}]
...
\subsubsection*{Negative definiteness in stationary points}
...
\label{sec:amle}
To find the maximizing parameters we differentiate \eqref{eq:loglik} w.r.t.\ $\vPsi$ and equate to zero while assuming $\nu$ known and constant. The 1.\ order derivative can be seen in equation \eqref{eq:dloglik}. Equating to zero is equivalent to
\begin{align}
  \vec{0}
  &= \frac{k\nu}{2} \vPsi^{-1}
    - \sum_{i=1}^k \frac{\nu + n_i}{2}
