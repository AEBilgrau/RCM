...
The \ref{suppB} show the significant GO-terms at significance level \Sexpr{dlbcl.par$go.alpha.level} for each module.
...
<<survival_analysis, echo=FALSE, results='hide', fig.height=1.2*7, fig.width=2*7, fig.pos="tb", fig.scap = "", fig.cap="\\it The top row shows $95\\%$ and $99\\%$ CI for the hazard ratio in the multiple Cox proportional hazards model. The bottom row show Kaplan-Meier estimates (and $95\\%$ CI) of the yellow dichotomized eigengene.">>=
@
...
<<one_dimensional_loglik, echo=FALSE, fig.height=1.5, fig.width=5, fig.pos="tb", ig.scap = "", fig.cap = "\\it The one-dimensional likelihood, the first and second derivatives as a function of $\\psi$.">>=
@
...
$
  ...
  \big\},
$
associated to each stationary point $vPsi_i$ are open sets in set of all positive semi-definite matrices $\calS_+$.
