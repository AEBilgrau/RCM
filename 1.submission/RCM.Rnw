...
A coexpression network (or weighted correlation network) analysis integrating all datasets was considered.
...
\fig{\Sexpr{dlbcl_plot}}{0.8\textwidth}{Top left: dendrograms of the hierarchical clustering, the identified modules, and the correlation matrix. Top right: the correlation network as laid out by the Fruchterman-Reingold algorithm \citep{Fruchterman1991} using edge-weights. The nodes are colored after the identified modules. The edge colors follow the color key. If the edge weight is numerically less than \Sexpr{round(qan, 3)}, corresponding to the top $95 \%$, the edge is suppressed. Bottom: A Hierarchical edge bundling representation of the network. Edge colors follow the color key. Again, only edges with a weight numerically larger than \Sexpr{round(qan, 3)} are plotted.
}
...
To further investigate the low ICC, we refitted the RCM on the subset of the \Sexpr{length(the.genes)} genes in the \Sexpr{the.module} module which yielded an
...
<<survival_analysis, echo=FALSE, results='hide', fig.height=1.2*7, fig.width=2*7, fig.pos="tb", fig.scap = "", fig.cap=paste("The top row shows $95\\%$ and $99\\%$ CI for the hazard ratio for each eigengene in the multiple Cox proportional hazards model containing all eigengenes. The bottom row shows Kaplan-Meier estimates (and $95\\%$ CI) of the overall survival for patients stratified by the dichotomized", the.module, "eigengene.")>>=
@

From the survival analysis, the \Sexpr{the.module} module appeared particularly interesting since it marked a gene cluster identifying DLBCL patients with significantly improved outcome.
Therefore a manual screening of the \Sexpr{length(the.genes)} genes within the \Sexpr{the.module} module was performed.
...
CHIT3L1 (also known as YKL40) has been linked to the AKT anti-apoptotic signaling pathway in glioblastoma \cite{Francescone2011} and thus high YKL40 is associated with poor outcome,
...
Suppose $Y$ is a random variable denoting the classes $1$ through $k$ and suppose $\vx$ is a random vector of the explanatory variables.
Recall that LDA and QDA estimate the class $y$ by maximizing
\begin{align*}
  P(Y = y | \vX = \vx) =
    \frac{\pi_y f(\vx | Y = y)}
...
We thank Martin Raussen and Jon Johnsen for their assistance on some of the mathematical proofs.
