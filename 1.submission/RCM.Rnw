...
\begin{frontmatter}
\title{Estimation of a common covariance matrix for multiple classes with applications in meta- and discriminant analysis\thanksref{T1}}
...
\ead[label=e1]{abilgrau@math.aau.dk}},
...
\ead[label=e3]{mboegsted@dcm.aau.dk}
}
...
We derive basic properties and estimators of the model and compare their properties.
...
Since the number of parameters increases quadratically in $p$ and the sample covariance matrix becomes singular when $p > n$ a plethora of shrinkage and regularization estimators have been proposed to combat the accompanying problems by effectively increasing the degrees of freedom. These examples include the graphical LASSO and ridge estimation of the precision matrix \citep{Friedman2008, VanWieringen2014}.
...
We wanted to use these studies in combination with data from our own laboratory to arrive at a good estimate of the covariance matrix whilst accounting for and assessing inter-study variation.
...
With the reparameterization $\vPsi = (\nu - p - 1)\vSigma$
in the inverse Wishart distribution of \eqref{eq:RCM} the common expected covariance matrix is
\begin{align}
  \label{eq:expcovar}
  \vSigma = \bbE[\vSigma_i] = \frac{\vPsi}{\nu-p-1} \text{ for } \nu > p + 1.
\end{align}
...
Thus, the true underlying covariance matrix $\vSigma$ and the homogeneity parameter $\nu$ are the effects of interest to be estimated in this paper unless stated otherwise.
...
\noindent The proofs have been deferred to Appendix \ref{sec:proofs}.
While the likelihood is not concave in $\vPsi$ we are able to show the existence and uniqueness of a global maximum in $\vPsi$.
...
To do this, the model is simply fitted a large number of times $N$ on datasets re-sampled under $H_0$ with permuted class labels to get $\hat{\nu}_0^{(1)}, ..., \hat{\nu}_0^{(N)}$.
...
\begin{align}
  ...
  \label{eq:ICCexprs}
\end{align}
This follows from the definition of the ICC which is the ratio of the between-study variation $\var(\Sigma_{ij})$ and the total variation $\var(S_{ij})$ of a single pair of any variables.
...
From the model, known results of the inverse Wishart distribution, cf.\ \citep{Cook2011, Rosen1988},
\begin{align}
...
We benchmark the proposed estimators against the known truth. The benchmarking measure used is the following weighted sum of squared errors,
\begin{align*}
...
For each $n = \Sexpr{min(par.ne$n.obs)}, \ldots, \Sexpr{max(par.ne$n.obs)}$, the weighted sum of squared errors for each estimator, $\text{SSE}(\hvSigma)$, were computed for $\Sexpr{par.ne[["n.sims"]]}$ datasets and the median of these values are seen in Figure \ref{fig:numerical_experiment_plot} as function of the number of samples in each dataset $n_i$.

<<numerical_experiment_plot, echo=FALSE, results='markup', fig.pos="tb", fig.height=7/2, fig.width=7, out.width = ".8\\linewidth", fig.scap = "", fig.cap=paste0("The median SSE of ", par.ne$n.sims, " simulations as a function of the number of samples $n_i$ in each of the ", par.ne$k, " studies.")>>=
@
...
Algorithm \ref{alg:RCM} and the different estimators are implemented in the statistical programming language R \cite{R} with core functions in \texttt{C++} using packages Rcpp and RcppArmadillo \citep{Eddelbuettel2011, RcppArmadillo}.
...
\subsection{DLBCL meta-analysis}
Diffuse large B-cell lymphoma (DLBCL) is an aggressive cancer subtype accounting for approximately $30\%-58\%$ of all non-Hodgkin's lymphoma (NHL) \citep{Project1997} which itself constitutes about $90 \%$ of all lymphomas.
...
Ten large-scale DLBCL gene expression studies were downloaded and preprocessed using custom brainarray chip definition files (CDF) \citep{Dai2005} and RMA-normalization using the R-package \texttt{affy} \citep{affy}.
...
The downloaded data together with  a data set from our own laboratory (GSE56315 \citep{DykaerBoegsted2015}) yields a total of \Sexpr{rowSums(dlbcl.dims)["Samples"]} samples with study sizes in the range \Sexpr{paste(range(dlbcl.dims["Samples",]), collapse = "-")}.
...
A co-expression network (or, weighted correlation network) analysis integrating all datasets was considered.
...
The tree was arbitrarily pruned at a height which produce \Sexpr{length(unique(dlbcl.modules))} modules named by colors.
Figure \ref{\Sexpr{dlbcl_plot_2}} shows these results. A heat-map, the hierarchical tree, and the identified modules are seen at the top left.
The top-right shows a graphical representation of the matrix better illustrating clusters and their relations.
The bottom plot shows the graph radially laid out and using hierarchical edge bundling \citep{Holten2006} where the edges are drawn to follow the hierarchical tree.
Table \ref{tab:dlbcl_mod_tab} shows the top genes within each module.
As seen e.g.\ in the green module, genes from the same gene family are clustered together.

To further investigate the low ICC, we refitted the RCM on the subset of the \Sexpr{length(orange.genes)} orange genes which yielded an
$\text{ICC}(\Sexpr{round(orange.rcm$nu, 2)}) = \Sexpr{round(with(orange.rcm, ICC(nu, nrow(Psi))),3)}$.
Subsequently the model was repeatedly fitted on \Sexpr{length(orange.genes)} randomly chosen genes \Sexpr{length(rand.rcm)} times to gauge the size of the ICC without the selection bias.
This resulted in a mean ICC (lower quartile, upper quartile) of \Sexpr{round(mean(sapply(rand.rcm, get.ICC)),3)} (\Sexpr{round(summary(sapply(rand.rcm, get.ICC))[c("1st Qu.", "3rd Qu.")],3)})
suggesting that, while there is indeed a significant selection bias, there seems to be a high study homogeneity by low ICC under randomly selected genes.

The test for null hypothesis of no study heterogeneity, $\nu = \infty$, is clearly rejected with a $p$~value of \Sexpr{signif(get.TestPValue(homogeneity.rcm, orange.rcm), 2)}.
The mean (sd) of the fitted $\hat{\nu}$ on the \Sexpr{length(homogeneity.rcm)} permuted datasets was $\Sexpr{round(mean(sapply(homogeneity.rcm, "[[", "nu")),1)}, (\Sexpr{round(sd(sapply(homogeneity.rcm, "[[", "nu")),2)})$ compared to $\hat{\nu}= \Sexpr{round(orange.rcm$nu, 2)}$ in the observed dataset.
...
The module eigengene is simply the first principal of the expression matrix of the module which can thus be represented by a linear combination of the module genes.
Hence, the module eigengene is a weighted average of the module genes.
For the most interesting modules, the Kaplan-Meier estimates were computed for groups arising when dichotomizing the values of the corresponding eigengene as above or below the median value.
These results are seen in Figure \ref{fig:survival_analysis}.


<<survival_analysis, echo=FALSE, results='hide', fig.height=1.2*7, fig.width=2*7, fig.pos="tb", fig.scap = "", fig.cap="The top row shows $95\\%$ and $99\\%$ CI for the hazard ratio in the multiple Cox proportional hazards model. The bottom row shows Kaplan-Meier estimates (and $95\\%$ CI) of the orange dichotomized eigengene.">>=
@

From the survival analysis, the orange module appeared particularly interesting since it marked a gene cluster identifying DLBCL patients with significant improved outcome.
Therefore a manual screening of the 16 genes within the orange module was performed.
...
The inflammation and modulated activity of the immune system are further suggested by genes ORM1, PLA2G2D, PLA2G7, IL18, and IGSF6.
...
Some of the remaining, MMP9, ALDH1A1, PTGDS, ADAMDEC1, HSD11B1, APOC1, and CYP27B1 are involved in metalloproteinase degradation and lipid metabolism.
MMP9 in particular is known to have a central role in proliferation, migration, differentiation, angiogenesis, apoptosis, and host defenses and thus high YKL40 is associated with poor outcome.
...
The manual screening and GO-analysis results further corroborates that the identified modules are biologically meaningful and that RCM provides a useful estimate of the covariance.
...
\end{align*}
over $y$, where $\vX | Y = y$ is assumed to be $p$-dimensional gaussian distributed, i.e.\
\begin{align*}
...
f(\vx | Y = y)
  ...
  &= \frac{ \big|\vPsi\big|^{\frac{\nu}{2}} \Gamma_p\!\left(\frac{\nu + 1}{2}\right) }
          { \pi^{-\frac{n}{2}}
            ...
            \Gamma_p\!\left(\frac{\nu}{2}\right)}.
\end{align*}
The derivation of which is analogous to Appendix \ref{sec:marginalization}.
The matrix determinant lemma, $|\vec{A} + \vec{u}\vPsi^\top| = (1 + \vPsi^\top\vec{A}\vec{u})|\vec{A}|$, can then be applied to simplify the expression and speed up the computations \citep{Ding2007}.
Note that HDA generalizes the LDA to have a multivariate $t$ distribution.
In fact it becomes a multivariate $t$-distribution when the sample sizes of each class is $1$.
Multivariate $t$-distributions have before been considered for discriminant analysis, cf.\ \citet{Andrews2011}.
Note that while the classifier is multivariate $t$-distributed, the estimation procedure assumes correlated observations within each class.


\subsubsection*{Benchmarking of HDA}
We designed four different scenarios to test and identify where HDA can be expected to perform similarly, better, or worse than LDA and QDA as gauged by the misclassification risk. The simulation experiment was inspired by the one seen in \citet{Friedman1989}.

In all four scenarios, we generated for $p = \Sexpr{par.xda$p.dims}$ a training dataset of $n = \Sexpr{par.xda[["n.obs"]]}$ observations belonging to $k = \Sexpr{par.xda[["K"]]}$ classes.
...
The \Sexpr{par.xda[["K"]]} covariance matrices were chosen to be either (a) equal and spherical, (b) unequal and spherical, (c) equal and highly elliptical, and (d) unequal and highly elliptical.
In scenario (a), $\vSigma_1 = \vSigma_2 = \vSigma_3 = \vI$.
In scenario (b), $\vSigma_k = k \vI$.
In scenario (c), the covariance matrices are equal, $\vSigma_1 = \vSigma_2 = \vSigma_3$, and chosen such that the square root of the $d$ eigenvalues are equidistant on the interval from 10 to 1 and a randomly (uniformly) oriented orthonormal basis is used for all components \citep{Friedman1989}.
...
Each simulation setup was repeated $\Sexpr{par.xda[["n.runs"]]}$ times and the mean and standard error, $\sqrt{\hat{p}(1-\hat{p})/n}$‚ of the misclassification risks were computed.
...
While the improvements are modest, the article demonstrates a potentially advantageous way of modelling the inter-study variability by a hierarchical random effects model.
...
We thank Martin Raussen and Jon Johnsen for thier assistance on some of the mathematical proofs.
The helpful statistical comments from Steffen Falgreen were much appreciated. The technical assistance from Alexander Schmitz, Julie S.\ B\o{}dker, Ann-Maria Jensen, Louise H. Madsen, and Helle H\o{}holt is also greatly appreciated.
...
\sdescription{The documents and other needed files to perform the analyses to reproduce this article. See the \texttt{README} file.}
\end{supplement}
...
\end{align*}
\normalsize
It it easy to find values $\psi$, $n_i$ and $\nu$ for which $\ell''(\psi) > 0$.
...
The multivariate gamma function $\Gamma_p$ is log-convex as can seen using the following characterization,
\begin{align}
  \label{eq:multigamma}
...
Before we restate the lemmas and proposition and their proofs, we see that the reparameterisation of the RCM is irrelevant.
...
Suppose there exists countably many stationary points $\vPsi_1, \vPsi_2, ...$.
...
\begin{align*}
  ...
  \big\},
\end{align*}
associated to each stationary point $\vPsi_i$ which is open when $\vPsi_i$ is a maximum\citep[Lemma 4.1]{Khalil2002}.
By Lemma \ref{lem:negativesdefinite}, $\vPsi_i$ is a always a maximum and hence all $A_i$ are open sets in the set of all positive semi-definite matrices $\calS_+$.
This partitions the space $\calS_+$ into $N$ disjoint, open sets.
However, this is only possible if $A_i = A_j = \calS_+$ for all $i$ and $j$ and thus there is only a single basin of attraction and maximum of $\ell$.
\end{proof}
...
\end{align*}
Clearly, the first term goes to $-\infty$ whenever an eigenvalue $\lambda_t \to 0$.
...
Hence, if $\lambda_t \to 0$ then
\begin{align*}
  \ell(\vPsi_t) \leq \frac{k\nu}{2}\log|\vPsi_t| + C_2 \to -\infty,
\end{align*}
...
The analog to $\vec{y}^\top \vec{A}\vec{y} = \sum_{ij} A_{ij} y_i y_j < 0$ in our vector space then becomes
\begin{align*}
...
\end{align*}
and without loss of generality assume that $\vPsi = \vI$. Hence, the likelihood equation \eqref{eq:loglik2} equated to zero, becomes
\begin{align*}
  k\nu\vI = \sum_a(n_a + \nu)(\vI + \vS_a)^{-1}
\end{align*}
which implies (by multiplication by $\vY$ on each side) that
\begin{align}
  k\nu\tr(\vY^2)
...
provided that
$\lim_{l \to \infty} (I - \vPsi^{-1}\vS_i)^l = \vec{0}$
for all $i$.
