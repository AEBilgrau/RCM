...
\begin{abstract}
We propose a hierarchical random effects model for a common covariance matrix in cases where multiple classes are present.
...
Diffuse large B-cell lymphoma (DLBCL) is an aggressive cancer subtype accounting for approximately $30\%-58\%$ of all non-Hodgkin's lymphomas (NHL) \citep{Project1997} which itself constitutes about $90 \%$ of all lymphomas.
...
The upper right of Figure~\ref{fig:dlbcl_plot} shows suggested functions of the modules primarily based on the GO analysis.
