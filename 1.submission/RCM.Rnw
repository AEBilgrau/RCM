...
\begin{frontmatter}
\title{Estimation of a common covariance matrix for multiple classes with
       applications in meta- and discriminant analysis\thanksref{T1}}
...
Bottom: A Hierarchical edge bundling representation of the network \citep{Holten2006} where edges loosly follow the dendrogram. Edge colors follow the color key. Again, only edges with a weight numerically larger than \Sexpr{round(dlbcl.par$threshold, 3)} are plotted.
}
...
The bottom plot shows the graph radially laid out and using hierarchical edge bundling \citep{Holten2006} where the edges are guided by the hierarchical tree.
...
Let $\vY = \sum_{(i,j)} y_{ij}\vE^{ij}$ be a symmetric matrix in the vector space where $\vY \neq \vec{0}$.
...
From the second derivative, this amounts to showing in that every stationary point,
\small
\begin{align}
