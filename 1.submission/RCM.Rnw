...
\kwd{meta-analysis}
\kwd{gaussian graphical modeling}
...
\begin{restatable}[Concavity in $\nu$]{proposition}{propositionConcavityInNu}
  ...
  For fixed positive semi-definite $\vPsi$, the log-likelihood function \eqref{eq:loglik}
  is concave in $\nu$.
\end{restatable}

\noindent Note, all proofs have been deferred to Appendix \ref{sec:proofs}.
...
\noindent This result follows from the following two lemmas.

\begin{restatable}{lemma}{lemmaOne}
\label{lem:elltominusinfty}
If there exists an eigenvalue $\lambda_i$ of $\vPsi$ such that $\lambda_i \to 0$ or   $\lambda_i \to \infty$, then $\ell(\vPsi) \to -\infty$ for $\nu$ fixed and $n_\bullet = \sum_{a=1}^k n_a \geq p$.
%   If $\nu$ is fixed and $n_\bullet = \sum_{a=1}^k n_a \geq p$ then $\ell(\vPsi) \to -\infty$ whenever $\exists \lambda_i : \lambda_i \to 0 \vee \lambda_i \to \infty$, where $\lambda_i$ is an eigenvalue of $\vPsi$.
\end{restatable}
...
This combined with the observation that $\ell(\vPsi) \to -\infty$ whenever an eigenvalue $\lambda_i \to 0$ or $\lambda_i \to \infty$ of Lemma \ref{lem:elltominusinfty} implies Proposition \ref{prop:uniquemax} and the existence and uniqueness of a global maximum.
...
Suppose $\hvPsi_{(t)}$ be the current estimate of $\vPsi$.
...
Let the ICC be given by
\begin{align}
  \text{ICC}(\nu)
  = \frac{1}{\nu-p}.
  \label{eq:ICCexprs}
\end{align}
This definition is motivated by considering the ratio of the between-study variation and the total variation.
Consider observations from \eqref{eq:RCM}. We temporarily abuse our notation and let
\begin{align*}
...
\begin{align*}
  \text{SSE}(\hvSigma) = \sum_{i \leq j} \frac{(\hat{\Sigma}_{ij}- \Sigma{ij})^2}{\var(\Sigma{ij})}
  \quad \text{ where } \quad
 \var(\Psi_{ij}) = n(\Sigma{ij}^2 + \Sigma{ii}\Sigma{jj}).
\end{align*}
...
For example, standard correlation network analyses can now be employed to the estimated common correlation matrix $\hat{\vec{R}}$ across all studies.
Here, we only outline one possible downstream analysis of the estimated covariance.
To identify sub-graphs, we used agglomerative hierarchical clustering with \Sexpr{capitalize(dlbcl.clust$method)}-linkage and distance measure defined as 1 minus the absolute value of the correlation.
The tree was arbitrarily pruned at a height which produce \Sexpr{length(unique(dlbcl.modules))} sub-graphs (or, modules) named by colors.
...
The module eigengene is simply the first principal of the expression matrix of the module which can thus be representation by a linear combination of the module genes.
...
From the survival analysis, the orange module appeared particular interesting since it marked a gene cluster identifying DLBCL patients with significant improved outcome.
...
The manual screening and GO-analysis results further corroborates that the identified modules are biologically meaningful and that RCM provides a useful estimates of the covariance.
...
Note that HDA generalizes the LDA to have a multivariate $t$-like distribution.
...
The technically possible esimates for extrem $n/p$ ratios does not nessearily imply a good estimate is achieved.
...
\sdescription{Lists of gene module memberships, auxiliary information, and the significant GO-terms for each identified module.}
\end{supplement}
...
Next, we show the uniqueness of the maxima.
...
\begin{align*}
  ...
  \big\},
\end{align*}
associated to each stationary point $\vPsi_i$ are open sets \citep[Lemma 4.1]{Khalil2002} %or \citep[Theorem 8.6]{Terrell2009})
in set of all positive semi-definite matrices $\calS_+$.
...
To du so, the matrix cookbook by \citet{Petersen2008} is a useful reference.
