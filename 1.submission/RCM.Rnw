...
We wished to use these studies in combination with data from our own laboratory to arrive at a good estimate of the covariance matrix whilst accounting for and assessing inter-study variation.
...
\noindent The proofs of these propositions has been deferred to Appendix \ref{sec:proofs}.
...
\noindent Lemma \ref{lem:negativesdefinite} tells that the Hessian in any stationary point (where $\frac{\partial\ell}{\partial\vPsi} = \vec{0}$) is negative definite, and hence every stationary point is a local maxima.
...
We also derived an approximate MLE using a first order approximation. This deriviation has been deferred to Appendix \ref{sec:amle}.
...
\Procedure{fitRCM}{$\vS_1, ..., \vS_k, n_1, ..., n_k, \hvPsi_{(0)}, \hat{\nu}_{(0)}, \varepsilon$}
  ...
  \For {$t = 1, 2, 3, ...$}
    \State {Set $\hvPsi_{(t)} \gets U\!\left(\hvPsi_{(t-1)}, \hat{\nu}_{(t-1)}\right)$}
    \State {Set $\hat{\nu}_{(t)} \gets \argmax_\nu \ell\!\left(\hvPsi_{(t)}, \nu\right)$}
    \State {Set $l_{(t)} \gets \ell\!\left(\hvPsi_{(t)}, \hat{\nu}_{(t)}\right)$}
...
We introduce a descriptive statistic analogous to the intra-class correlation coefficient (ICC) \citep{Stanish1983} well known from ordinary meta-analysis to better determine what constitute large values of $\nu$.
...
<<numerical_experiment_plot, echo=FALSE, results='markup', fig.pos="tb", fig.height=7/2, fig.width=7, out.width = ".8\\linewidth", fig.scap = "", fig.cap=paste0("The estimated mean weigthed sum of squared errors (MSSE) based on ", par.ne$n.sims, " simulations, as a function of the number of samples $n_i$ in each study.")>>=
@
...
This document was prepared with \texttt{knitr} \citep{Xie2013} and LaTeX. To reproduce this document see \ref{suppA}.
...
A co-expression network (or, weighted correlation network) analysis integrating all datasets was wanted.
...
The tree was arbitrarily pruned at a height which yielded \Sexpr{length(unique(dlbcl.modules))} sub-graphs (or, modules) named by colors.
...
\ref{suppB} show the significant GO-terms at significance level \Sexpr{dlbcl.par$go.alpha.level} for each module in which the most GO-terms appear highly relevant to the pathology of DLBCL.
...
The module eigengene is simply the first principal component applied to the expression matrix of the module which can thus be representation by a linear combination of the module genes.
...
<<survival_analysis, echo=FALSE, results='hide', fig.height=1.2*7, fig.width=2*7, fig.pos="tb", fig.scap = "", fig.cap="The top row shows $95\\%$ and $99\\%$ CI for the hazard ratio in the multiple Cox proportional hazards model. The bottom row show Kaplan-Meier estimates (and $95\\%$ CI) of the yellow dichotomized eigengene.">>=
@
...
Genes CHI3L1 , CHIT1, and LYZ are related to chitin degradation and suggests activated immune system response and inflammation.
...
The recent advances in such regularized techniques which allows for analysis of large covariance matrices has unfortunately diminished the focus on collecting larger sample.
...
\sdescription{Lists of gene module membersships, auxiliary information, and the significant GO-terms for each identified module.}
\end{supplement}
...
\end{align*}
\normalsize
We see, that $\ell''(\psi) \to - \infty$ for $\psi \to 0^+$ and $\ell''(\psi) \to 0$ for $\psi \to \infty^-$.
The likelihood and its derivative with $k = 1$ for appropriately chosen parameters is seen in Figure \ref{fig:one_dimensional_loglik}.% which is generated by the \texttt{R}-code:
The log-likelihood $\ell$ is not log-concave as the second derivative is not always negative.
...
<<one_dimensional_loglik, echo=FALSE, fig.height=1.5, fig.width=5, fig.pos="tb", ig.scap = "", fig.cap = "The one-dimensional likelihood, the first and second derivatives as a function of $\\psi$.">>=
@
...
Suppose there exists two (or more) countably many stationary points $\vPsi_1, \vPsi_2, ...$.
...
All gradient curves (that is, solution curves to $\dot{\vPsi}(t) = \nabla \ell(\vPsi(t))$) must then converge toward exactly one of the stationary points where $\ell$ monotonically increase along each curve.
...
We approximate by the first order expansion $(l = 1)$,
\begin{align*}
  \vec{0}
