...
\runtitle{Estimation of a common covariance matrix for multiple classes with applications in meta- and discriminant analysis}
...
\begin{algorithm}[tb]
\caption{Pseudo-code for the RCM coordinate ascent estimation procedure}
\label{alg:RCM}
\begin{algorithmic}[1]
...
This document was prepared with \texttt{knitr} \citep{Xie2013} and LaTeX. To reproduce this document see Supplementary material \ref{suppA}.
...
\stitle{knitr document}
\slink[url]{http://people.math.aau.dk/\%7eabilgrau/RCM/SuppA/}
\sdescription{The \texttt{knitr} and \texttt{.R} documents and other needed files to perform the analyses to reproduce this article.}
\end{supplement}

\begin{supplement}
\sname{Supplement B}\label{suppB}
\stitle{Identified modules and GO analysis}
\slink[url]{http://people.math.aau.dk/\%7eabilgrau/RCM/SuppB/}
\sdescription{Complete lists of gene module membersships, auxiliary information, and the significant GO-terms for each identified module.}
\end{supplement}
...
\section{Marginalization of covariance}
