...
To further investigate the low ICC, we refitted the RCM on the \Sexpr{length(orange.genes)} orange genes only which yielded $\text{ICC}(\Sexpr{round(orange.rcm$nu, 2)}) = \Sexpr{round(with(orange.rcm, ICC(nu, nrow(Psi))),3)}$.
Subsequently the model repeatedly fitted on \Sexpr{length(orange.genes)} randomly chosen genes \Sexpr{length(rand.rcm)} times to gauge the size of the ICC without the selection bias.
This gave and mean ICC (lower quartile, upper quartile) of \Sexpr{mean(sapply(rand.rcm, get.ICC))} (\Sexpr{summary(sapply(rand.rcm, get.ICC))[c("1st Qu.", "3rd Qu.")]})
suggesting that while there indeed is a significant selection bias, there seems to be a high study homogeneity by low ICC under randomly selected genes.
...
The mean (sd) of the fitted $\hat{\nu}$ on the \Sexpr{length(homogeneity.rcm)} permuted datasets was $\Sexpr{round(mean(sapply(homogeneity.rcm, "[[", "nu")),1)}, (\Sexpr{round(sd(sapply(homogeneity.rcm, "[[", "nu")),2)})$.
...
We thank Martin Raussen and Jon Johnsen for help with some of the mathematical proofs.
