...
\begin{subfigure}{0.5\textwidth}
  ...
  \label{fig:3b}
\end{subfigure}
\caption{Heatmap and iGraph for the estimated correlation matrices of the top 300 genes for the DLBCL data using either the EM method. The network is cut at a height producing 5 clusters}
\label{fig:3}
\end{figure}
...
Next, the modules were screened for biological relevance using GO (Gene Ontology) Biological Process, Molecular Function, and Cellular Component as well as REACTOME and KEGG pathway enrichment analysis. Since we pre-selected the top 300 genes by variance, the enrichment analysis was done using only these as the background genes. Top genes for each module are shown in Table \ref{tab:top.genes.em}, while results of the GO enrichment analysis at significance level \Sexpr{dlbcl.par$go.alpha.level} for each of the modules are shown in Supplementary Table \ref{tab:enrichment.em}. Inspection of the enrichment analysis \{tableA2} and most connected genes \ref{table3} allowed us to hypothesize that the coral module is involved in "tissue development", the Skyblue module is involved in "metastasis", the orchid module involved in "immune regulation", and the olivegreen module involved in "immune surveillance".  

\input{table.top.genes.em}
%\input{table.enrichment.em}

From the GO annotation and survival analysis the olivegreen module appeared particularly interesting, as we notice a strong involvement of immune response and an association between high value of the eigengene expression and poor survival. Eventually making these patients candidates for experimental immunotherapies. Several of the genes, e.g. S100A8, S100A9, CD14, and CD163 with the highest connectivity in this module have been associated to immunotherapy \citep{fulmer2008, Cheng2008, pmid28331613}. As prominent examples S100A8 (MRP8; calgranulin A) and the gene S100A9 (MRP14; calgranulin B) appear in the list.  This is interesting
...
(Fulmer, 2008). Notably, in mice injected with lymphoma cells, knockout
of S100A9 resulted in greater tumor in_ltration of T-cells and less accumulation
of MDS cells than that seen in wild-type mice (Cheng et al., 2008).
...
Finally, we compared the network analyses based on the covariance matrix obtanied by the EM to that obtained by the Pool methods. The upper row of Figure \ref{fig:A5} shows the heatmap and associated network modules for the Pool method, when the dendogram is cutted at \Sexpr{dlbcl.par$n.modules} modules. For the Pool method, we chose for each module the same color as the module of the EM based clustering with most overlapping genes. In the lower row of Figure \res{fig:A5} a tangle diagram was constructed and the cophenitic correlation was calculated. We noticed a great overlap between the modules, but a low cophenetic correlation. With background in the simulation we anticipate the Pool method has lower efficiency than the EM method. This implies the signal from the tightly regulated modules are so strong that it manifest itself for both methods. However, if we look at the less correlated intramodular connections the noise plays a larger role leading to a less clear separation between the modules for the Pool method. This can have potential biological implications, when regulating hub genes resulting in intra module cascades of reactions.    
